...
const SPEED_RATINGS = Object.freeze(['fast', 'medium', 'slow']);
const MODEL_INTELLIGENCE = Object.freeze({
  'google/nano-banana-2': {
    notIdealFor: ['ultra-final identity renders', 'complex surreal backgrounds', 'project setup without Vertex credentials'],
    skinTonePerformance: 'strong - reliable on melanin-rich skin when identity refs are clean',
    identityLockStrength: 4,
    backgroundRealism: 3,
    creativeRange: 3,
    speedRating: 'fast',
    costZAR: 0.74,
    ...
    characterNote: 'Best fast-pass route for referenced character iterations before a Pro final.',
    warningNote: 'Use Nano Banana Pro when identity fidelity is the deciding factor.'
  },
  'google/nano-banana-pro': {
    notIdealFor: ['very complex backgrounds without scene refs', 'surreal/conceptual exploration without identity refs', 'cheap bulk drafts'],
    skinTonePerformance: 'excellent - particularly strong on melanin-rich skin with direct references',
    identityLockStrength: 5,
    backgroundRealism: 3,
    creativeRange: 3,
    speedRating: 'medium',
    costZAR: 1.48,
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Best default final route for Aisha, Leah, Claudia, and Vanya when identity refs are active.',
    warningNote: 'Complex environments benefit from scene refs or a FLUX route.'
  },
  'google/imagen-3-text-only': {
    notIdealFor: ['identity-locked character shots', 'direct reference workflows', 'precise wardrobe matching'],
    skinTonePerformance: 'moderate - useful for no-ref scenes, weaker for exact character skin fidelity',
    identityLockStrength: 2,
    backgroundRealism: 4,
    creativeRange: 5,
    speedRating: 'fast',
    costZAR: 0.74,
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Best Google-credit option for no-reference scene concepts and environment exploration.',
    warningNote: 'Do not use for exact character work with refs.'
  },
  'openai/gpt-image-2': {
    notIdealFor: ['cheap bulk drafts', 'simple no-ref images', 'Google-credit-only spend lanes'],
    skinTonePerformance: 'strong - good semantic preservation when refs and constraints are explicit',
    identityLockStrength: 4,
    backgroundRealism: 4,
    creativeRange: 4,
    speedRating: 'slow',
    ...
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Best fal.ai route when semantic edits, text/layout reasoning, or complex reference intent matters.',
    warningNote: 'Costs more and should be reserved for complex instructions.'
  },
  'openai/gpt-image-1.5': {
    notIdealFor: ['primary final renders', 'lowest-cost drafts', 'when GPT Image 2 is available'],
    skinTonePerformance: 'good - usable fallback with explicit skin and identity constraints',
    identityLockStrength: 3,
    backgroundRealism: 3,
    creativeRange: 3,
    speedRating: 'medium',
    costZAR: 1.48,
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Fallback GPT Image route for older experiments and special compatibility.',
    warningNote: 'Prefer GPT Image 2 for new complex image work.'
  },
  'black-forest-labs/flux-2-pro': {
    notIdealFor: ['direct reference-image identity lock', 'multi-reference character matching', 'utility edits'],
    skinTonePerformance: 'strong - tasteful realism, but exact identity depends on written constraints',
    identityLockStrength: 4,
    backgroundRealism: 5,
    creativeRange: 4,
    speedRating: 'medium',
    ...
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Best for complex environments, cinematic architecture, and no-ref hero scenes.',
    warningNote: 'This registry route is text-to-image only; use Google or GPT routes for direct identity refs.'
  },
  'black-forest-labs/flux-2-max': {
    notIdealFor: ['budget-sensitive runs', 'bulk iteration', 'direct reference-image identity lock'],
    skinTonePerformance: 'strong - premium realism with careful prompt constraints',
    identityLockStrength: 4,
    backgroundRealism: 5,
    creativeRange: 5,
    speedRating: 'slow',
    ...
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Highest-fidelity no-ref final route for polished cinematic hero images.',
    warningNote: 'Expensive final-only route; avoid for iteration.'
  },
  'bytedance/seedream-5-lite': {
    notIdealFor: ['ultra-premium final finish', 'lowest-risk exact identity finals', 'simple utility edits'],
    skinTonePerformance: 'good - practical multi-reference fallback with explicit constraints',
    identityLockStrength: 3,
    backgroundRealism: 3,
    creativeRange: 4,
    speedRating: 'medium',
    ...
    requiresRefs: false,
    supportsNegativePrompt: true,
    characterNote: 'Useful middle route for multi-reference blending when Google is not the right lane.',
    warningNote: 'Not the strongest identity or highest-finish route.'
  },
  'fal/qwen-image-2-edit': {
    notIdealFor: ['text-to-image generation', 'final character renders', 'multi-reference blending'],
    skinTonePerformance: 'limited - utility edit route, not a final identity model',
    identityLockStrength: 2,
    backgroundRealism: 2,
    creativeRange: 2,
    speedRating: 'fast',
    ...
    requiresRefs: true,
    supportsNegativePrompt: true,
    characterNote: 'Best for quick image-to-image utility edits, not final character creation.',
    warningNote: 'Requires a reference image and supports only one reference in this route.'
  }
});
...
const IMAGE_MODELS = Object.freeze([
  {
    ...
    ...MODEL_INTELLIGENCE['google/nano-banana-2'],
    strengths: [
  ...
  },
  {
    ...
    ...MODEL_INTELLIGENCE['google/nano-banana-pro'],
    strengths: [
  ...
  },
  {
    ...
    ...MODEL_INTELLIGENCE['google/imagen-3-text-only'],
    strengths: [
  ...
  },
  {
    ...
    supportsEditing: true,
    maxReferenceImages: 10,
    ...MODEL_INTELLIGENCE['openai/gpt-image-2'],
    strengths: [
  ...
  },
  {
    ...
    ...MODEL_INTELLIGENCE['openai/gpt-image-1.5'],
    strengths: [
  ...
  },
  {
    ...
    supportsEditing: false,
    maxReferenceImages: 0,
    ...MODEL_INTELLIGENCE['black-forest-labs/flux-2-pro'],
    strengths: [
  ...
  },
  {
    ...
    supportsEditing: false,
    maxReferenceImages: 0,
    ...MODEL_INTELLIGENCE['black-forest-labs/flux-2-max'],
    strengths: [
  ...
  },
  {
    ...
    supportsEditing: true,
    maxReferenceImages: 6,
    ...MODEL_INTELLIGENCE['bytedance/seedream-5-lite'],
    strengths: [
  ...
  },
  {
    ...
    ...MODEL_INTELLIGENCE['fal/qwen-image-2-edit'],
    strengths: [
...
function clonePublicModel(model, env = process.env) {
  ...
  return {
    ...
    notIdealFor: [...(model.notIdealFor || model.avoidWhen || [])],
    skinTonePerformance: model.skinTonePerformance || '',
    identityLockStrength: Number(model.identityLockStrength || 0),
    backgroundRealism: Number(model.backgroundRealism || 0),
    creativeRange: Number(model.creativeRange || 0),
    speedRating: model.speedRating || 'medium',
    costZAR: Number(model.costZAR || estimateZar(model.costEstimateUsd, env) || 0),
    requiresRefs: Boolean(model.requiresRefs),
    supportsNegativePrompt: Boolean(model.supportsNegativePrompt),
    characterNote: model.characterNote || '',
    warningNote: model.warningNote || null,
...
function assertRegistryIntegrity() {
  const ratingFields = ['identityLockStrength', 'backgroundRealism', 'creativeRange'];
  for (const model of IMAGE_MODELS) {
    ...
    if (!Array.isArray(model.notIdealFor) || model.notIdealFor.length === 0) {
      throw new Error(`Model ${model.id} must declare notIdealFor guidance.`);
    }
    for (const field of ratingFields) {
      const value = Number(model[field]);
      if (!Number.isInteger(value) || value < 1 || value > 5) {
        throw new Error(`Model ${model.id} has invalid ${field}; expected integer 1-5.`);
      }
    }
    if (!SPEED_RATINGS.includes(model.speedRating)) {
      throw new Error(`Model ${model.id} has invalid speedRating.`);
    }
    if (!Number.isFinite(Number(model.costZAR)) || Number(model.costZAR) <= 0) {
      throw new Error(`Model ${model.id} must declare positive costZAR.`);
    }
    if (typeof model.skinTonePerformance !== 'string' || !model.skinTonePerformance.trim()) {
      throw new Error(`Model ${model.id} must describe skinTonePerformance.`);
    }
    if (typeof model.requiresRefs !== 'boolean' || typeof model.supportsNegativePrompt !== 'boolean') {
      throw new Error(`Model ${model.id} must declare boolean reference and negative-prompt support.`);
    }
    if (model.warningNote !== null && typeof model.warningNote !== 'string') {
      throw new Error(`Model ${model.id} warningNote must be null or a string.`);
    }
  }
  return true;
}
