...
function aishaIsIntentionallySilent(silentReactions = []) {
  return (Array.isArray(silentReactions) ? silentReactions : []).some(item => {
    ...
    return /\b(anchor|anchoring|authority|continuity)\b/i.test(visibleState)
      || /\b(holding authority|holding continuity|continuity anchor|intentionally silent|planned silence|quiet anchor|tracking the room|watching for the premise)\b/i.test(reason);
  });
}
