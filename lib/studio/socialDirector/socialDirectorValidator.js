...
function foodAnswerDodged(normalized = {}, context = {}) {
  ...
  return /\b(planning discussion is paused|what is the immediate need|anyone need a quick fuel|check in on sustenance|before we dive into tomorrow)\b/.test(visible);
}
...
function operationalJargonRisk(normalized = {}) {
  ...
  return /\b(parameters were clear|within those parameters|operational parameters|current operational parameters|conditions are optimal|no extraneous data|no extraneous elements|content selection|specific suggestions|aligns with current focus|the plan is set|functioning within parameters|efficiency dictates|build specifications|technical specifications|technical specs|draft the specs|adjust the specs|pulse animation parameters|load times|optimized|optimize for that specific interaction|proceed with that configuration|commit to the visual|current build|exact red hex code|we can implement that|standard approach|proceed with that framework|necessary warmth|ensure the content|capability was present|taste under pressure|variable that needs calibration|move to the next item|style guide|update the style guide|remove the red pulse element|current objectives|feedback is noted|perform usefulness|evidence of completion is the only metric|avoid further debate)\b/.test(visible);
}
...
function frustrationIgnored(normalized = {}, context = {}) {
  const current = normalizeKey(context.userMessage || '');
  if (!/\b(stressed|stress|dumb|this feels dumb|starting to feel dumb|frustrated|annoyed|annoying|this is bad|this sucks|repeating yourself|keep repeating|stop repeating|answer normally|respond normally|talk normally)\b/.test(current)) return false;
  const visible = normalizeKey([
    normalized.roomBeat,
    ...(normalized.speakers || []).map(item => item.text),
    ...(normalized.stateUpdates?.notes || [])
  ...
  if (/\b(repeating yourself|keep repeating|stop repeating|answer normally|respond normally|talk normally)\b/.test(current)
    && /\b(the ask is simple|actual problem you need solved|state it clearly|address only what is necessary|what is the actual problem|only what is necessary to fix it)\b/.test(visible)) {
    return true;
  }
