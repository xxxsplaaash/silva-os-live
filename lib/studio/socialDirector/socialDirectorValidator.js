...
function practicalTopic(context = {}) {
  ...
  const joined = `${current} ${recent}`.trim();
  const terseFollowup = /^(where do i start|what is the objective|bruh|bro|lol|ok|okay|now what|what next|how)$/i.test(String(context.userMessage || '').trim());

  if (/\b(muscle|muscles|fitness|workout|working out|gym|lift|lifting|strength|bulk|train|training|exercise|reps|sets|protein)\b/.test(joined)) {
    return {
      kind: 'fitness',
      required: ['muscle', 'fitness', 'workout', 'training', 'train', 'gym', 'lift', 'strength', 'sets', 'reps', 'protein', 'sleep', 'recover', 'start', 'beginner']
    };
  }
  if (/\b(food|hungry|lunch|dinner|snack|eat|coffee|tea)\b/.test(joined)) {
    return { kind: 'food', required: ['food', 'eat', 'hungry', 'lunch', 'dinner', 'snack'] };
  }
  if (/\b(work|design|plan|planning|build|campaign|logo|creative|idea|schedule|calendar|content)\b/.test(joined)) {
...
function refusalOfAllowedTopic(value = '', context = {}) {
  ...
  if (!topic) return '';
  const text = String(value || '');
  if (/\b(we are not discussing|not discussing|not the objective|not a current objective|outside (the )?scope|focus is required|maintain focus on the current objectives|personal fitness routines|not here for fitness|not discussing personal fitness|not relevant to the room)\b/i.test(text)) {
    return topic.kind;
  }
  return '';
}
...
function ignoresPracticalTopic(normalized = {}, context = {}) {
  ...
  ].join(' ')).replace(/\b(we are not discussing|not discussing|not the objective|outside the scope|focus is required|personal fitness routines|not here for fitness|not relevant to the room)\b/g, ' ');
