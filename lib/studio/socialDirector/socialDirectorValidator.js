...
function designAnswerPunted(normalized = {}, context = {}) {
  ...
  const punts = /\b(what s the core message|what is the core message|what are the non negotiables|define the visual language|before we iterate|let s hear the prompt|what is the core issue|what are we trying to land|what message are we trying)\b/.test(visible);
  const concrete = /\b(mark|wordmark|black|white|red|accent|pulse|shape|spacing|contrast|signal|field|badge|monogram|line weight|negative space)\b/.test(visible);
