...
const { blindAttributionForLine, evaluateVisibleResponse } = require('./visibleResponseQuality');
...
function genericSilentReasonRisk(value = '') {
  const reason = normalizeKey(value);
  if (!reason) return false;
  return /\b(waiting|standing by|standby|monitoring|observing|watching|listening)\b/.test(reason)
    && !/\b(because|while|until|before|without|this turn|current|claim|premise|structure|taste|temperature|correction|practical|selected voice|selected voices|answer)\b/.test(reason);
}
...
function referencesText(context = {}) {
  return (Array.isArray(context.references) ? context.references : Array.isArray(context.messageReferences) ? context.messageReferences : [])
    .map(item => `${item?.speakerId || item?.speakerName || ''}: ${item?.text || item?.content || ''}`)
    .join('\n');
}

function shortSessionFollowupRequested(value = '') {
  const text = normalizeKey(value || '');
  return /\b(20|twenty)\s+minutes?\b/.test(text)
    && /\b(turn|make|convert|compress|that|this|it|version|short|shorter|only|limited)\b/.test(text);
}

function practicalTopic(context = {}) {
  const current = normalizeKey(context.userMessage || '');
  const recent = normalizeKey(`${referencesText(context)}\n${recentTurnsText(context)}`);
  ...
  if (/\b(muscle|muscles|fitness|workout|working out|gym|lift|lifting|strength|bulk|train|training|exercise|reps|sets|protein)\b/.test(current)) {
    return {
      kind: 'fitness',
      required: ['muscle', 'fitness', 'workout', 'training', 'train', 'gym', 'lift', 'strength', 'sets', 'reps', 'protein', 'sleep', 'recover', 'start', 'beginner', 'squat', 'hinge', 'push', 'pull', 'core', 'row', 'plank', 'dead bug', 'round', 'timer', 'minute']
    };
  }
  if (/\b(food|hungry|lunch|dinner|snack|eat|coffee|tea)\b/.test(current)) {
    return { kind: 'food', required: ['food', 'eat', 'hungry', 'lunch', 'dinner', 'snack', 'meal', 'sandwich', 'wrap', 'rice', 'eggs', 'egg', 'toast', 'banana', 'yogurt', 'yoghurt', 'soup', 'salad', 'leftovers', 'beans', 'lentils', 'tuna', 'fruit', 'oats', 'peanut butter', 'chicken', 'fish', 'cheese'] };
  }
  if (/\b(movie|film|watch tonight|what should we watch|what should the room watch)\b/.test(current)) {
    return { kind: 'movie', required: ['movie', 'film', 'watch', 'title', 'tonight', 'mood', 'arrival', 'spider verse', 'spiderverse', 'the menu', 'dune', 'mad max', 'heat', 'parasite', 'knives out', 'blade runner', 'thriller', 'comedy', 'horror', 'drama', 'animation', 'atmosphere'] };
  }
  if (/\b(work|design|plan|planning|build|campaign|logo|creative|idea|schedule|calendar|content|landing page|homepage|website|web page|hero|saas|visual direction|brand direction|black glass|red pulse)\b/.test(current)) {
    return { kind: 'work', required: ['work', 'design', 'plan', 'planning', 'schedule', 'tomorrow', 'day', 'task', 'block', 'owner', 'cleanup', 'gap', 'build', 'creative', 'idea', 'next', 'step', 'landing', 'page', 'hero', 'website', 'brand', 'style', 'visual', 'black', 'red', 'pulse', 'logo', 'mark', 'wordmark', 'monogram', 'contrast', 'shape', 'spacing', 'statement', 'template', 'tech'] };
  }
  ...
  if (shortSessionFollowupRequested(current) && /\b(muscle|muscles|fitness|workout|training|gym|strength|reps|sets|push ups|pushups|push|pull|rows|squats|hinges|plank|sessions?)\b/.test(recent)) {
    return {
      kind: 'fitness',
      required: ['muscle', 'fitness', 'workout', 'training', 'train', 'gym', 'lift', 'strength', 'sets', 'reps', 'protein', 'sleep', 'recover', 'start', 'beginner', 'squat', 'hinge', 'push', 'pull', 'core', 'row', 'plank', 'dead bug', 'round', 'timer', 'minute']
    };
...
function repeatedRecentAssistantRisk(normalized = {}, context = {}) {
  const current = normalizeKey(context.userMessage || '');
  if (/\b(preference|style|dashboard|landing page|brand|what changed|what was changed|difference|old|prior|previous|never said|did i)\b/.test(current)
    && /\b(actually|what changed|what was changed|difference|old|prior|previous|never said|did i|is)\b/.test(current)) {
...
function vanyaPhraseFamilies(value = '') {
  const text = normalizeKey(value || '');
  const families = [];
  const checks = [
    ['tiny-vanity', /\b(tiny vanity|massive discipline|we can work with that)\b/],
    ['first-round-proof', /\b(first round proves|second round earns|mirror can wait|day is moving)\b/],
    ['dramatic-reset', /\b(dramatic reset|dramatic rebrand|heroic rebrand|no heroic rebrand)\b/],
    ['no-drama', /\b(no drama|leave the ceremony outside|ceremony stays outside|without ceremony)\b/],
    ['thesis', /\b(turn(?:ing)? .* into a thesis|into a thesis|thesis)\b/]
  ];
...
function repeatedVanyaPhraseFamilyRisk(normalized = {}, context = {}) {
  const currentVanya = (Array.isArray(normalized.speakers) ? normalized.speakers : [])
    .filter(item => normalizeSpeakerId(item?.speakerId || '') === 'vanya')
    .flatMap(item => vanyaPhraseFamilies(item?.text || ''));
  if (!currentVanya.length) return '';
  const current = new Set(currentVanya);
  const recentVanyaFamilies = (Array.isArray(context.recentTurns) ? context.recentTurns : [])
    .filter(item => normalizeSpeakerId(item?.speakerId || '') === 'vanya')
    .slice(-4)
    .flatMap(item => vanyaPhraseFamilies(item?.text || item?.content || ''));
  return recentVanyaFamilies.find(family => current.has(family)) || '';
}
...
function staleTopicAnswer(normalized = {}, context = {}) {
  ...
  const movieSpecificDirection = /\b(arrival|spider verse|spiderverse|the menu|dune|mad max|heat|parasite|knives out|blade runner|thriller|comedy|horror|drama|animation|atmosphere|social commentary|quiet pressure|voltage|bite)\b/.test(visible);
  ...
  if (/\b(lunch|eat for lunch|what should i eat for lunch|dinner|snack|meal)\b/.test(current)
    ...
    && /\b(before training|after training|pre training|post training|training fuel|session fuel|feed the session|feed the workout|not the performance|lets you move|lets you train|move without feeling heavy|workout timing)\b/.test(visible)) {
    return 'fitness';
  }
...
function voiceLockIssuesForSpeaker(speaker = {}, rawText = '') {
  ...
  if (!speakerId) return [];
  const rawVisible = String([speaker.text || '', rawText || ''].join('\n'));
  ...
  const allNames = Object.values(selfNames).flat();
  const rawLabelPattern = /^\s*(?:A\.?\s*I\.?\s*S\.?\s*H\.?\s*A\.?|Aisha|Vanya|Leah|Claudia|Grok|Gerhard)\s*[:.-]\s+/im;
  if (rawLabelPattern.test(rawVisible)) {
    issues.push(`voice-lock:speaker-name-cue:${speakerId}`);
  }

  const nameCuePattern = allNames.length
    ? new RegExp(`\\b(${allNames.join('|')})\\b\\s+(you|your|the|that|this|come|stop|said|says|is|was|still|needs|named|outlined|tracks|anchors|thinks|would|will|can|keeps|wants|suggested|gave|frames|here|speaking|checking)\\b`)
    : null;

  if (nameCuePattern && nameCuePattern.test(visible) && !issues.includes(`voice-lock:speaker-name-cue:${speakerId}`)) {
    issues.push(`voice-lock:speaker-name-cue:${speakerId}`);
  }
  ...
  const otherNames = Object.entries(selfNames)
    .filter(([id]) => id !== speakerId)
    .flatMap(([, names]) => names);
  const otherNameCuePattern = otherNames.length
    ? new RegExp(`\\b(${otherNames.join('|')})\\b\\s+(you|your|the|that|this|come|stop|said|says|is|was|still|needs|named|outlined|tracks|anchors)\\b`)
    : null;
  if (otherNameCuePattern && otherNameCuePattern.test(visible) && !issues.includes(`voice-lock:speaker-name-cue:${speakerId}`)) {
    issues.push(`voice-lock:speaker-name-cue:${speakerId}`);
  }
  ...
  if (/\b(i understand how you feel|makes sense to feel|its okay to feel|it is okay to feel|carrying a lot|process your feelings|your feelings are valid|safe space|hold space|emotional container|trauma informed|breathe through it|honor your experience|allowed to feel|give yourself permission|sit with that emotion)\b/.test(visible)) {
  ...
  if (speakerId === 'aisha'
    && /\b(let'?s get you started|that'?s a great|that'?s a solid|i recommend|you should start by|start by focusing on|here are some tips|maintain focus|current objectives|support your goal|journey)\b/.test(visible)) {
    issues.push('voice-lock:support-bot-aisha:aisha');
  }
  if (speakerId === 'vanya'
    && /\b(it sounds like|what i'?m hearing|let'?s unpack|gentle reminder|you are seen|you are heard|emotional container|process your feelings|carrying a lot)\b/.test(visible)) {
    issues.push('voice-lock:therapy-voice:vanya');
  }
  if (speakerId === 'leah'
    && /\b(from a strategic perspective|strategically speaking|leverage|optimize|stakeholder|brand positioning exercise|i agree with|that'?s a good point|we should consider|it is important to consider|interesting perspective)\b/.test(visible)) {
    issues.push('voice-lock:corporate-leah:leah');
  }
  if (speakerId === 'grok'
    && /\b(interesting question|good point|i agree|helpful approach|let'?s break it down|there are several factors|it depends on your goals|best practice)\b/.test(visible)) {
    issues.push('voice-lock:generic-grok:grok');
  }
  if (speakerId === 'leah'
    && /\b(this is trash|this is garbage|whoever approved|whoever made it should be embarrassed|no taste|idiot|stupid|embarrassing for you|delete it and apologize|ugly and worthless|that idea is pathetic|pathetic lazy|deserves to be laughed out|laughed out of the room)\b/.test(visible)) {
    issues.push('voice-lock:hostile-leah:leah');
  }
  if (speakerId === 'claudia'
    && /\b(prioritized action item checklist|action item checklist|actionable next steps with a timeline|timeline and deliverables|define stakeholders|align deliverables|circle back|sync offline|project plan|milestone tracker|risk register|deliverable owners|ownership matrix|kpi checkpoint|roadmap ownership|roadmap and ownership)\b/.test(visible)) {
    issues.push('voice-lock:project-manager-claudia:claudia');
  }
  if (speakerId === 'grok'
    && /\b(only rational mind|explain the flaw in tiny words|obviously everyone else|well actually|only i seem capable|as the genius|intellectual superior|beneath my intellect|i alone understand|humanity has peaked|peaked in stupidity|field research in stupidity|room is doing field research)\b/.test(visible)) {
...
function frustrationIgnored(normalized = {}, context = {}) {
  ...
  if (/\b(repeating yourself|keep repeating|stop repeating)\b/.test(current)
    && /\b(the loop is clear|let s reset|lets reset|repeating is just another way to dodge|energy thins when we refuse to land the point|pick one topic and give a single actionable step|single actionable step for it)\b/.test(visible)) {
    return true;
  }
  if (/\b(answer normally|what should i do today|what do i do today)\b/.test(current)) {
    ...
    if (/\b(pick one thing|one hour of focused work|single project|lets just get one thing done|let s just get one thing done|then we can see where we are|its okay to feel that way|it is okay to feel that way)\b/.test(visible)) return true;
...
function aishaOverAnsweredOrdinaryPractical(normalized = {}, context = {}) {
  const topic = practicalTopic(context);
  if (!topic) return false;
  if (String(topic.kind || '') === 'work' && /\b(preference|style|color|dashboard|landing page|brand|claim|contradiction|record|changed|prior|previous)\b/.test(normalizeKey(context.userMessage || ''))) {
    return false;
  }
  const addressed = normalizeSpeakerId(directAddressTarget(context.userMessage || ''));
  if (addressed === 'aisha') return false;
  const speakers = Array.isArray(normalized.speakers) ? normalized.speakers : [];
  if (!speakers.some(item => item.speakerId === 'aisha')) return false;
  const aisha = speakers.find(item => item.speakerId === 'aisha');
  const text = normalizeKey(aisha?.text || '');
  if (/\b(record|prior|previous|contradict|contradiction|claim|receipt|evidence|active|superseded|changed)\b/.test(text)) return false;
  return true;
}
...
function validateDirectorOutput(output = {}, context = {}) {
  ...
  const repeatedVanyaFamily = repeatedVanyaPhraseFamilyRisk(normalized, context);
  if (repeatedVanyaFamily) issues.push(`voice-lock:repeated-vanya-phrase:${repeatedVanyaFamily}`);
  ...
  if (aishaOverAnsweredOrdinaryPractical(normalized, context)) issues.push('voice-lock:overhelpful-aisha:aisha');
  ...
  for (const [index, speaker] of normalized.speakers.entries()) {
    ...
    const blindAttribution = blindAttributionForLine(speaker.text);
    if (blindAttribution.wordTotal >= 1 && !blindAttribution.identifiable) {
      issues.push(`voice-lock:blind-attribution:${speaker.speakerId}`);
    } else if (
      blindAttribution.wordTotal >= 1
      && blindAttribution.identifiable
      && blindAttribution.speakerId
      && blindAttribution.speakerId !== speaker.speakerId
    ) {
      issues.push(`voice-lock:blind-attribution:${speaker.speakerId}`);
    }
  }

  for (const item of normalized.silentReactions) {
    if (!CHARACTER_IDS.includes(item.speakerId)) issues.push(`invalid-silent-speaker:${item.speakerId || 'missing'}`);
    if (!compactText(item.reason || '', 140)) issues.push(`silent-reaction-missing-reason:${item.speakerId || 'missing'}`);
    else if (genericSilentReasonRisk(item.reason)) issues.push(`silent-reaction-generic-reason:${item.speakerId || 'missing'}`);
  }
