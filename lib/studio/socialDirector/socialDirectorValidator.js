...
function operationalJargonTextRisk(text = '') {
  ...
  if (/\b(draft schedule based on today|draft schedule based on todays|based on today s outcomes|based on todays outcomes|key deliverables|morning session|energy levels are accounted for|everyones energy levels|any immediate needs or concerns for tomorrow|room theatre|theatrical planning|stop performing|customer support|polished dodge|mostly fake sounding|fake sounding|less doctrine|performance art|sustenance is a parameter|is a parameter not|execute|evidence beats another round|failed answer wearing a badge|perform a job title)\b/.test(visible)) return true;
