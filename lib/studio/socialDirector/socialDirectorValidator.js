...
function voiceLockIssuesForSpeaker(speaker = {}, rawText = '') {
  ...
  if (/\b(really valid|valid reaction|that's a valid reaction|that s a valid reaction|that's a fair reaction|that s a fair reaction|fair reaction|feelings are valid|that sounds valid|appreciate you sharing|holding space|safe space|gentle reminder|you are seen|you are heard)\b/.test(visible)) {
    issues.push(`voice-lock:generic-warmth:${speakerId}`);
  }
  if (/\b(here comes my joke|here comes my sarcastic joke|sarcastic joke|to be funny|humorously speaking|insert joke|jokes aside)\b/.test(visible)) {
    issues.push(`voice-lock:announced-humor:${speakerId}`);
  }
  if (/\b(carrying a lot|process your feelings|your feelings are valid|safe space|hold space|emotional container|trauma informed|breathe through it|honor your experience)\b/.test(visible)) {
    issues.push(`voice-lock:therapy-voice:${speakerId}`);
  }

  if (speakerId === 'aisha'
    && /\b(a i s h a is|aisha is an artificial intelligence|designed to facilitate|multi agent conversational|conversational coordination|as an ai system|as an artificial intelligence)\b/.test(visible)) {
    issues.push('voice-lock:wikipedia-aisha:aisha');
  }
  if (speakerId === 'leah'
    && /\b(this is trash|whoever approved|no taste|idiot|stupid|embarrassing for you|delete it and apologize|ugly and worthless)\b/.test(visible)) {
    issues.push('voice-lock:hostile-leah:leah');
  }
  if (speakerId === 'claudia'
    && /\b(prioritized action item checklist|action item checklist|define stakeholders|align deliverables|circle back|sync offline|project plan|milestone tracker|risk register|deliverable owners)\b/.test(visible)) {
    issues.push('voice-lock:project-manager-claudia:claudia');
  }
  if (speakerId === 'grok'
    && /\b(only rational mind|explain the flaw in tiny words|obviously everyone else|as the genius|intellectual superior|beneath my intellect|i alone understand)\b/.test(visible)) {
...
function validateDirectorOutput(output = {}, context = {}) {
  ...
  if (context.impulsePlan?.enforceSelectedSpeakers === true && plannedSpeakerIds.length) {
    ...
    const unplanned = (Array.isArray(output?.speakers) ? output.speakers : [])
      .map(item => normalizeSpeakerId(item?.speakerId || ''))
      .filter(Boolean)
      .find(id => !plannedSet.has(id));
