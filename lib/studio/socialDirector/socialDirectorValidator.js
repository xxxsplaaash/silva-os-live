...
function genericStatusReport(normalized = {}, context = {}) {
  ...
  return /\b(work is proceeding|current tasks|operational status is green|no immediate blockers|status is green|everyone is engaged|tracking the workout|next steps are logged|incline push ups are logged|backpack rows|log reps|data is clear|excuses are not)\b/.test(visible);
}
