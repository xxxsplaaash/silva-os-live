...
function frustrationIgnored(normalized = {}, context = {}) {
  ...
  return /\b(we use process to avoid commitment|structure itself becomes the excuse|capability is not the issue|capability is not the weak point|pattern is clear|stress comes from|mistaking polish for progress|actual work not the presentation|actual work not presentation|name the feeling not the function|concrete mood|stress is noted|proceed with that clarity)\b/.test(visible);
}
