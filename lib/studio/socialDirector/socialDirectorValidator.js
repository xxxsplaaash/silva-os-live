...
function staleTopicAnswer(normalized = {}, context = {}) {
  ...
  if (/\b(actual tension|tension in this room|room tension)\b/.test(current)
    && /\b(film|movie|arrival|spider verse|spiderverse|the menu|heat|knives out|specific suggestions|content selection|parameters for the film|what are the specific suggestions|consensus is clear|title follows|quiet intensity|strong world)\b/.test(visible)) {
    return 'prior-topic';
  }
...
function genericStatusReport(normalized = {}, context = {}) {
  ...
  return /\b(work is proceeding|current tasks|operational status is green|no immediate blockers|status is green|everyone is engaged|tracking the workout|next steps are logged|incline push ups are logged|backpack rows|log reps|log the count|data is clear|data will be the only acceptable output|acceptable output now|execute the first move|no more discussion|the path is clear|excuses are not)\b/.test(visible);
}
...
function foodAnswerDodged(normalized = {}, context = {}) {
  ...
  return /\b(planning discussion is paused|what is the immediate need|anyone need a quick fuel|check in on sustenance|before we dive into tomorrow|defining tomorrow s objective|lunch is a secondary concern|secondary concern)\b/.test(visible);
}

function designAnswerPunted(normalized = {}, context = {}) {
  const current = normalizeKey(context.userMessage || '');
  if (!/\b(logo|sharper logo|logo direction|brand direction|sharper.*direction)\b/.test(current)) return false;
  const visible = normalizeKey([
    normalized.roomBeat,
    ...(normalized.speakers || []).map(item => item.text),
    ...(normalized.stateUpdates?.notes || [])
  ].join(' '));
  const punts = /\b(what s the core message|what is the core message|what are the non negotiables|define the visual language|define the core elements|before we iterate|let s hear the prompt|what is the core issue|what are we trying to land|what message are we trying|what specific visual language|communicates edge|needs to be clear enough|cuts through the noise|real edge)\b/.test(visible);
  const concrete = /\b(mark|wordmark|black|white|red|accent|pulse|shape|spacing|contrast|signal|field|badge|monogram|line weight|negative space|sharp serif|condensed|geometric|stroke|cut|warning|silhouette)\b/.test(visible);
...
function operationalJargonRisk(normalized = {}) {
  ...
  return /\b(parameters were clear|within those parameters|operational parameters|current operational parameters|conditions are optimal|no extraneous data|no extraneous elements|content selection|specific suggestions|aligns with current focus|the plan is set|functioning within parameters|efficiency dictates|build specifications|technical specifications|technical specs|draft the specs|adjust the specs|adjust the design parameters|design parameters|parameters are updated|parameters updated|configuration is now active|configuration is now the standard|system configuration|update the system configuration|corrected dashboard settings|dashboard settings|active setting|active selection|current standard|pulse animation parameters|load times|optimized|optimize for that specific interaction|proceed with that configuration|proceed with white editorial|clear enough for implementation|commit to the visual|current build|exact red hex code|we can implement that|standard approach|proceed with that framework|necessary warmth|ensure the content|capability was present|taste under pressure|variable that needs calibration|move to the next item|objective is execution|style guide|update the style guide|remove the red pulse element|map the accent placement|key interactive elements|current objectives|feedback is noted|perform usefulness|evidence of completion is the only metric|avoid further debate)\b/.test(visible);
}
...
function movieAnswerTooThin(normalized = {}, context = {}) {
  ...
  const hasSpecificDirection = /\b(arrival|spider verse|spiderverse|the menu|dune|mad max|heat|parasite|knives out|comfort|tension|spectacle|thriller|comedy|horror|action|drama|animation|quiet pressure|voltage|bite)\b/.test(visible);
  const puntsToUser = /\b(what are the options|pick the mood first|what options|depends on what you want|tell me what mood)\b/.test(visible);
...
function frustrationIgnored(normalized = {}, context = {}) {
  ...
  if (/\b(repeating yourself|keep repeating|stop repeating|answer normally|respond normally|talk normally)\b/.test(current)
    && /\b(the ask is simple|actual problem you need solved|state it clearly|address only what is necessary|what is the actual problem|only what is necessary to fix it|what is the one thing you need to do next|what part of the last answer|room needs a clear ask|identify the core requirement|current exchange is not yielding)\b/.test(visible)) {
    return true;
  }
  if (/\b(answer normally|what should i do today|what do i do today)\b/.test(current)) {
    if (/\b(what is the one thing you need to do next|what part of the last answer|room needs a clear ask|state the actual problem|identify the core requirement|current exchange is not yielding|clear ask to move forward)\b/.test(visible)) return true;
  ...
  if (/\b(bruh|bro|wtf)\b/.test(current)
    && /\b(the objective is the execution|stick to the plan|we have the structure|that s the objective|thats the objective)\b/.test(visible)) {
    return true;
  }
