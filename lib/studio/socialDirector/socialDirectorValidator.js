...
function frustrationIgnored(normalized = {}, context = {}) {
  ...
  if (/\b(bruh|bro|wtf)\b/.test(current)
    && /\b(the objective is the execution|objective is the first move|the objective is the first move|stick to the plan|we have the structure|that s the objective|thats the objective)\b/.test(visible)) {
    return true;
  }
