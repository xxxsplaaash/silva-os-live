...
function qualityInvalidIssue(issue = '') {
  return /^(allowed-topic-refusal|false-objective-claim|topic-ignored|recent-repeat-risk|stale-topic-answer|generic-status-report|takeover-for-ordinary-topic|feels-task-router-risk|repeated-point-risk)/.test(String(issue || ''));
}
