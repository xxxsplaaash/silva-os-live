...
function enforceImpulsePlanOnOutput(output = {}, impulsePlan = null) {
  ...
  const plannedIds = Array.isArray(impulsePlan.selectedSpeakers)
    ...
    : [];
  const plannedOrder = new Map(plannedIds.map((id, index) => [id, index]));
  ...
  const filtered = rawSpeakers
    ...
    .sort((a, b) => {
      const ai = plannedOrder.has(normalizeSpeakerId(a?.speakerId || '')) ? plannedOrder.get(normalizeSpeakerId(a?.speakerId || '')) : 99;
      const bi = plannedOrder.has(normalizeSpeakerId(b?.speakerId || '')) ? plannedOrder.get(normalizeSpeakerId(b?.speakerId || '')) : 99;
      return ai - bi;
    });
  const cappedSpeakers = (filtered.length ? filtered : rawSpeakers).slice(0, maxSpeakers);
