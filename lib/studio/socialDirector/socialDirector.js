...
function qualityInvalidIssue(issue = '') {
  return /^(allowed-topic-refusal|false-objective-claim|topic-ignored|recent-repeat-risk|stale-topic-answer|generic-status-report|operational-jargon|generic-advice-column|social-question-ignored|continuity-question-ignored|takeover-for-ordinary-topic|feels-task-router-risk|repeated-point-risk)/.test(String(issue || ''));
}
