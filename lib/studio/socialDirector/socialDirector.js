...
function enforceImpulsePlanOnOutput(output = {}, impulsePlan = null) {
  ...
  const plannedSilentBySpeaker = new Map(plannedSilent
    .map(item => [normalizeSpeakerId(item?.speakerId || ''), item])
    .filter(([id]) => id));
  ...
  const silentReactions = [
    ...
    ...requiredSilent
      ...
      .map(speakerId => quietReactionForDroppedSpeaker(speakerId, plannedSilentBySpeaker.get(speakerId)))
...
function failureCategoryForAttempt(attempt = null) {
  ...
  if (issues.includes('empty-response-content') || issues.includes('no-content') || /\b(no-content|no usable room response|no usable)\b/i.test(reason)) return 'no-content';
...
function attemptSpeakerOrder(attempt = null) {
  const output = attempt?.validation?.output || attempt?.parsed || {};
  return (Array.isArray(output.speakers) ? output.speakers : [])
    .map(item => normalizeSpeakerId(item?.speakerId || ''))
    .filter(Boolean)
    .slice(0, 5);
}

function attemptResponseMode(attempt = null) {
  const output = attempt?.validation?.output || attempt?.parsed || {};
  return compactText(output.responseMode || '', 40);
}

function attemptAccepted(attempt = null) {
  return attempt?.validation?.ok === true;
}

function attemptIssueCount(attempt = null) {
  return Array.isArray(attempt?.validation?.issues) ? attempt.validation.issues.length : 0;
}

function attemptDiagnostics(attempt = null) {
  if (!attempt) {
    return {
      status: '',
      category: '',
  ...
  return {
    status: compactText(attempt.parseStatus || '', 80),
    category: compactText(attemptAccepted(attempt) ? '' : failureCategoryForAttempt(attempt), 80),
    accepted: attemptAccepted(attempt),
    issueCount: attemptIssueCount(attempt),
    responseMode: attemptResponseMode(attempt),
    speakerOrder: attemptSpeakerOrder(attempt)
  };
}

function validationWithAttemptDiagnostics(validation = {}, { first = null, repair = null } = {}) {
  const firstAttempt = attemptDiagnostics(first);
  const repairAttempt = attemptDiagnostics(repair);
  const firstAttemptIssues = Array.isArray(first?.validation?.issues) ? first.validation.issues : [];
  const repairAttemptIssues = Array.isArray(repair?.validation?.issues) ? repair.validation.issues : [];
  return {
    ...validation,
    firstAttemptStatus: firstAttempt.status,
    repairAttemptStatus: repairAttempt.status,
    firstAttemptCategory: firstAttempt.category,
    repairAttemptCategory: repairAttempt.category,
    firstAttemptAccepted: firstAttempt.accepted,
    repairAttemptAccepted: repairAttempt.accepted,
    firstAttemptIssueCount: firstAttempt.issueCount,
    repairAttemptIssueCount: repairAttempt.issueCount,
    firstAttemptResponseMode: firstAttempt.responseMode,
    repairAttemptResponseMode: repairAttempt.responseMode,
    firstAttemptSpeakerOrder: firstAttempt.speakerOrder,
    repairAttemptSpeakerOrder: repairAttempt.speakerOrder,
    firstAttemptIssues,
    repairAttemptIssues
  };
}
...
function responseEnvelope(output, validation, meta = {}, attempts = {}) {
  const enrichedValidation = validationWithAttemptDiagnostics(validation, attempts);
  const runtime = safeDiagnostics(meta.response || {});
  const publicValidation = enrichedValidation?.output?.socialCues
    ? { ...enrichedValidation, output: { ...enrichedValidation.output, socialCues: undefined } }
    : enrichedValidation;
  const envelope = {
    ok: true,
    mode: MODE,
    ...
    qualityAccepted: enrichedValidation?.accepted === true && enrichedValidation?.fallbackUsed !== true && !(enrichedValidation?.issues || []).length,
    repairedByRuntime: enrichedValidation?.repaired === true,
    qualityFailureCategory: compactText(meta.failureCategory || enrichedValidation?.failureCategory || '', 80),
    debugSummary: {
      ...
      failureCategory: compactText(meta.failureCategory || enrichedValidation?.failureCategory || '', 80),
      ...
      repaired: enrichedValidation?.repaired === true,
      fallbackUsed: enrichedValidation?.fallbackUsed === true,
      qualityAccepted: enrichedValidation?.accepted === true && enrichedValidation?.fallbackUsed !== true && !(enrichedValidation?.issues || []).length,
      qualityFailureCategory: compactText(meta.failureCategory || enrichedValidation?.failureCategory || '', 80),
  ...
  return {
    ...
    projectContext: {
      socialDirectorV1: {
        ...
        acceptanceRubric: acceptanceRubricFor(input),
        flags: input.flags,
...
async function runAishaDirector({ input, callAishaEngine, runtimeOptions, body, repair = null }) {
  ...
  const planned = parsed ? enforceImpulsePlanOnOutput(parsed, input.impulsePlan) : null;
  const validation = parsed
    ? validateDirectorOutput(planned, { userMessage: input.userMessage, recentTurns: input.recentTurns, references: input.references, impulsePlan: input.impulsePlan })
  ...
  return { response, content, parsed: planned || parsed, validation, parseStatus };
}
...
async function runSocialDirectorTurn({
  ...
  if (typeof callAishaEngine === 'function') {
    try {
        ...
        if (repair.validation.ok) {
          return {
            ...
            payload: responseEnvelope(repair.validation.output, {
              ...
              repairAttemptIssues: [],
              firstAttemptStatus: first?.parseStatus || '',
              repairAttemptStatus: repair?.parseStatus || '',
              repaired: true,
              fallbackUsed: false,
              failureCategory: ''
            }, {
      ...
      const firstConnected = first?.response?.aishaEngineConnected === true || first?.response?.aishaConnected === true;
      const errorAttempt = {
        response: {
          aishaEngineConnected: firstConnected,
          aishaConnected: firstConnected,
          engineMode: firstConnected ? (first?.response?.engineMode || 'production') : 'unavailable',
          ...
          diagnostics: {
            ...(first?.response?.diagnostics && typeof first.response.diagnostics === 'object' ? first.response.diagnostics : {}),
            rejectionReason: timedOut ? 'social-director-generation-timeout' : 'social-director-error'
          }
        },
        validation: { issues: [timedOut ? 'generation-timeout' : 'aisha-call-error'] },
        error: err
      };
      if (first) repair = errorAttempt;
      else first = errorAttempt;
    }
  }
  ...
  const fallbackValidation = validateDirectorOutput(fallbackOutput, { userMessage: input.userMessage, recentTurns: input.recentTurns, references: input.references, impulsePlan: input.impulsePlan });
  ...
  return {
    ...
    payload: responseEnvelope(fallbackValidation.output, {
      ...
      aishaTraceStatus: safeTraceStatus(failedAttempt?.response || {}),
      aishaTraceFailureReason: safeTraceFailure(failedAttempt?.response || {}),
