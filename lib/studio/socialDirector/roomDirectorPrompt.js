...
function hardRules(input = {}) {
  ...
  return [
    ...
    referencedShortWindow ? 'For a referenced fitness follow-up asking for a 20-minute version, answer exactly that: Claudia first with a timed mini-plan that compresses the referenced moves into categories like push, pull, legs, hinge, core. Do not restate the prior starter list. Vanya second only if she adds a fresh social pressure line. Do not answer with generic habit talk, "focused work", "45 seconds on, 15 seconds rest", or "no wasted motion".' : '',
...
function examplesText() {
  return [
    ...
    'GOOD: Claudia says "Twenty minutes: warm up for 3, then two rounds of push, pull, legs, hinge, and core. Write the lowest rep count down." Vanya says "Let the clock do the arguing; the ego can decorate later."'
  ].join('\n');
}
...
function repairGuidanceFor(input = {}, repair = {}) {
  ...
  if (/\b(hungry|lunch|dinner|snack|eat|food|meal|cook|recipe|carbonara|pasta)\b/i.test(input.userMessage || '')
    ...
    guidance.push(
      'Ordinary-lunch voice repair: Claudia must open with named food options plus one decision rule, such as eggs and toast, rice bowl, solid sandwich, leftovers with water, or boring baseline. Vanya must make the lunch or afternoon feel human with playful bite, such as personality-test pressure; no movement, training, performance, session-fuel, or generic nutrition-assistant language.'
    );
  }
