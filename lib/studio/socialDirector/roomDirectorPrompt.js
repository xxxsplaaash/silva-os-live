...
function positiveTargetsFor(input = {}) {
  ...
  if (referencedFitnessShortWindow(input)) {
    return [
      ...
      'Claudia should answer first with a concrete 20-minute version: named movement categories or moves, timer blocks, and one count to log. Compress the reference; do not restate the prior starter list.',
      'Never say "chosen exercises" or use 40/20, 45/15, or work/rest interval boilerplate for this follow-up.',
...
function hardRules(input = {}) {
  ...
  return [
    ...
    referencedShortWindow ? 'For a referenced fitness follow-up asking for a 20-minute version, answer exactly that: Claudia first with a timed mini-plan that compresses the referenced or recent workout structure into categories or named moves like push, pull, legs, hinge, core, chair squats, incline push-ups, backpack rows, dead bugs, plus one rep count to log. If the referenced card is Vanya social framing, preserve only the human context and still let Claudia land the plan first. Do not restate the prior starter list. Vanya second only if she adds a fresh social pressure line. Do not answer with generic habit talk, "chosen exercises", "focused work", "45 seconds on, 15 seconds rest", "40 seconds on, 20 seconds off", work/rest interval boilerplate, or "no wasted motion".' : '',
...
function repairGuidanceFor(input = {}, repair = {}) {
  ...
  if (hasIssue(/\b(product-topic-ignored:referenced-fitness|referenced-fitness)\b/i)) {
    guidance.push(
      'Referenced fitness repair: the referenced workout card is the anchor. If the user asks for a 20-minute version, Claudia must answer with a timed 20-minute mini-plan that compresses the referenced moves into named categories or moves before any social line; do not say "chosen exercises", do not use work/rest interval boilerplate, and do not restate the prior starter list. Vanya may add one fresh pressure line only.'
    );
  }
  ...
  if (referencedShortWindow && !guidance.some(line => /20-minute mini-plan/i.test(line))) {
    guidance.push(
      'Current turn is a referenced 20-minute fitness follow-up: answer the time constraint directly with a concrete mini-plan before any commentary; name the movement categories or moves, not "chosen exercises".'
    );
  }
  return guidance;
}
