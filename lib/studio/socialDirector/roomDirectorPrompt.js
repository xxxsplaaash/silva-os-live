...
function positiveTargetsFor(input = {}) {
  ...
  if (staleFitnessSocialPivot(input)) {
    return [
      ...
      'Answer the room temperature directly through distinct voices; do not answer with workouts, reps, food, recovery, or fitness motivation.',
...
function recentVisibleLineGuard(input = {}) {
  const recentLines = (Array.isArray(input.recentTurns) ? input.recentTurns : [])
    .filter(item => {
      const speakerId = normalizeSpeakerId(item?.speakerId || '');
...
function hardRules(input = {}) {
  ...
  return [
    ...
    staleFitnessPivot ? 'This turn is a social room check-in, not a training follow-up. Do not answer with workouts, reps, food, recovery, or fitness motivation; answer the current room temperature and social tension.' : '',
