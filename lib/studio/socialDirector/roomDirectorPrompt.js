...
function voiceSignatureContracts() {
  return [
    ...
    'Claudia floor: first step, timer, owner, sequence, count, or measurable next move.',
...
function attributionDriftContracts() {
  return [
    ...
    'Do not give Vanya Claudia machinery: timers, owners, numbered blocks, measurable next moves, or handoff logistics belong to Claudia unless Vanya adds an unmistakable human-temperature read.',
    'Do not give Claudia Vanya warmth: temperature, room dignity, breathable reset, social pressure, or emotional landing belongs to Vanya unless Claudia also names a concrete first step, timer, owner, count, or constraint.',
    'Do not give Leah Grok machinery or Claudia plans: Leah must land taste, status, edge, cultural pressure, or visual stakes, not premise faults, evidence audits, timers, owners, or generic next steps.',
...
const RECENT_REPEAT_FAMILY_RULES = Object.freeze([
  {
    ...
    instruction: 'Vanya must choose a new human-pressure image and avoid discipline/clock/ceremony phrasing.'
  },
  {
    speakerId: 'claudia',
    ...
    instruction: 'Claudia must use a different structure shape: one owner, one constraint, or one next measurable move.'
  },
  {
    speakerId: 'grok',
    label: 'track-reps / narrative-ambition fault',
    pattern: /\b(track reps|narrative ambition|otherwise you are just|premise fault|fault line|evidence beats|prove a point)\b/i,
    instruction: 'Grok must name a new premise consequence without track/proof/narrative-ambition language.'
  },
  {
    speakerId: 'leah',
    ...
    instruction: 'Leah must use a fresh taste verdict and avoid title/mood/world/status-pressure phrasing.'
  },
  {
    speakerId: 'aisha',
...
function examplesText() {
  return [
    ...
    'GOOD: Claudia says "Twenty minutes: warm up for 3, then two rounds of incline push-ups, backpack rows, split squats, hip hinges, and plank. Write the lowest rep count down." Vanya says "Let the clock do the arguing; the ego can decorate later."'
  ].join('\n');
}
