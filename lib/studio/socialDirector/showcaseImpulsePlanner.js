...
function recentTextOf(input = {}) {
  return (Array.isArray(input.recentTurns) ? input.recentTurns : [])
    .slice(-8)
...
function terseFollowupRequested(text = '') {
  return /^(where do i start|what is the objective|bruh|bro|wtf|ok|okay|now what|what next|how|ok but i only have 20 minutes|i only have 20 minutes|only have 20 minutes)$/i.test(compactText(text, 160).toLowerCase());
}
...
function categoryFor(input = {}) {
  ...
  if (/\b(muscles?|fitness|workout|gym|training|train|lunch|dinner|snack|hungry|eat|food|meal|cook|recipe|carbonara|pasta|plan|planning|schedule|tomorrow|logo|design|landing page|website|build|bug|provider|timeout|python|pdf|script|code|parser|parse|movie|film|watch|netflix|series|show|social media|instagram|tiktok|caption|post|disagreement|disagree)\b/i.test(text)) {
    return 'practical';
  }
...
function orderedSpeakersFor(category = 'normal', input = {}) {
  ...
  if (category === 'practical') {
    ...
    if (/\b(hungry|lunch|dinner|snack|eat|food|meal|cook|recipe|carbonara|pasta|plan|planning|schedule|tomorrow|workout|muscles?|gym|training|train)\b/i.test(text)) return ['claudia', 'vanya'];
