...
function roomTensionRequested(text = '') {
  return /\b(actual tension|tension in this room|room tension)\b/i.test(text);
}
...
function hasRecentFitnessContext(input = {}) {
  return /\b(muscles?|fitness|workout|gym|training|train|push-?ups?|pushups|rows?|squats?|lunges?|hinges?|planks?|core|reps?|sets?|rounds?|sessions?)\b/i.test(recentTextOf(input));
}
...
function topicClassFor(input = {}) {
  ...
  if (roomTensionRequested(text)) return 'room-tension';
...
function categoryFor(input = {}) {
  ...
  if (topicClass === 'room-tension') return 'normal';
...
function selectedLineJobFor(speakerId = '', input = {}) {
  ...
  if (shortSessionFollowupRequested(text) && /\b(muscles?|fitness|workout|gym|training|train|push-?ups?|rows?|squats?|hinges?|plank|reps?|sets?|sessions?)\b/i.test(recentTextOf(input))) {
    const jobs = {
      ...
      vanya: 'add one fresh human pressure line after Claudia; avoid recent mirror, first-round, clock, ego, heroic-rebrand, and solid-block imagery',
      leah: 'stay quiet unless taste pressure would improve the practical answer',
      claudia: 'compress the referenced workout into a 20-minute timer plan using named movement categories or moves plus one rep count; never say chosen exercises or work/rest intervals',
      grok: 'stay quiet unless there is a safety or premise fault'
    };
    return jobs[id] || speakerProfileFor(id).socialObjective;
  }
  if (terseFollowupRequested(text) && hasRecentFitnessContext(input)) {
    const jobs = {
      aisha: 'hold the record while the terse frustration stays inside the workout thread',
      vanya: 'read the frustration with bite, then keep the answer physical instead of emotional-reset language',
      leah: 'stay quiet unless taste pressure would improve the practical answer',
      claudia: 'answer with one concrete workout move, timer, rep count, or training-day line; no water or clear-a-surface reset',
      grok: 'challenge only if the user is chasing certainty before doing the first set'
    };
    return jobs[id] || speakerProfileFor(id).socialObjective;
  }
  if (/\b(hungry|lunch|dinner|snack|eat|food|meal|cook|recipe|carbonara|pasta)\b/i.test(text)) {
    ...
    const jobs = trainingAdjacent ? {
      ...
      claudia: 'open with at least two Claudia lunch anchors: one decision, boring baseline, eggs and toast, rice bowl, solid sandwich, or leftovers with water; name options before any caveat',
...
function currentPracticalBeatLabel(input = {}) {
  ...
  if (terseFollowupRequested(text) && hasRecentFitnessContext(input)) return 'fitness recovery';
...
function orderedSpeakersFor(category = 'normal', input = {}) {
  ...
  if (topicClass === 'room-tension') return ['vanya', 'leah', 'grok'];
...
function capFor(category = 'normal', input = {}) {
  ...
  if (topicClassFor(input) === 'room-tension') return { minSpeakers: 2, maxSpeakers: 3, responderCapReason: 'room-tension asks need a few distinct voices naming the pressure, not a full-room roll call' };
...
function buildShowcaseImpulsePlan(input = {}) {
  ...
  return {
    ...
    enforceSelectedSpeakers: category === 'practical' || category === 'direct' || category === 'emotional' || topicClass === 'room-tension',
