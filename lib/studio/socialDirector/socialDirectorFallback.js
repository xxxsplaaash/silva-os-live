...
function recentFitnessRecoveryShape(body = {}) {
  ...
  if (/\b(the objective is your actual ask: start building muscle|start with three full-body sessions a week|start at home this week|no heroic rebrand required|incline push-ups, backpack rows|split squats, hip hinges|recovered from rejected fitness refusal)\b/i.test(recent)) return 'first';
  return '';
}
...
function continuityClaimFallback(text = '') {
    ...
    return {
      ...
      speakers: [
        speaker('aisha', 'primary', 'precise continuity', prior
          ? `Yes: prior record was ${prior}; current record is ${active || 'not resolved yet'}.`
          : `Yes. I can see "${cited}" in the recent room record.`, 'Anchoring'),
...
function socialFallbackFor(message = {}, body = {}) {
  ...
  const continuityClaim = continuityClaimFallback(text);
  ...
  if (/\b(hungry|food|eat|eating|lunch|dinner|snack|meal|nutrition)\b/i.test(text)) {
    ...
    if (trainingFood) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Eat small enough that training still feels possible.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'If you train soon: banana and yoghurt. If you have more time: eggs and toast, or rice and chicken; add water.', 'Tracking next steps'),
          speaker('grok', 'closer', 'dry diagnostic', 'If it sits heavy, make it smaller next time. That is the whole experiment.', 'Tracking')
        ],
        silentReactions: [reaction('aisha', 'Anchoring'), reaction('leah', 'Watching')],
  ...
  if (/\b(repeating yourself|keep repeating|stop repeating|answer normally|respond normally|talk normally)\b/i.test(text)) {
    ...
    return {
      ...
      speakers: [
        ...
        speaker('grok', 'closer', 'dry diagnostic', 'A repeated answer is a failed answer wearing a badge. Good. Now we can stop rewarding it.', 'Tracking failure')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('leah', 'Watching')],
