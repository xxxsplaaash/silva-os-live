...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (currentTopic(text) === 'movie_watch' || currentTopic(text) === 'watch_next') {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'host with taste', 'New topic. Good. Pick the mood first: comfort, tension, or spectacle.', 'Reading the room'),
        speaker('leah', 'side', 'sharp taste', 'If it is tonight, I want one strong world, not wallpaper. Arrival for quiet pressure, Spider-Verse for voltage, The Menu if you want bite.', 'Holding critique'),
  ...
  if (currentTopic(text) === 'room_tension') {
    return {
      ...
      speakers: [
        ...
        speaker('grok', 'closer', 'dry diagnostic', 'Capability is not the weak point. Taste under pressure is.', 'Tracking failure')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('claudia', 'Tracking next steps')],
