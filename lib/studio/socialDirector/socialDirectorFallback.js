...
function recentFitnessRecoveryShape(body = {}) {
  ...
  if (/\b(no more loop|pick three training days|week one|boring enough to repeat|room energy, not a task queue|calendar first: monday|three repeatable training days|tiny vanity, massive discipline)\b/i.test(recent)) return 'second';
...
function continuityChangeFallback(text = '', body = {}) {
  ...
  if (active && prior) {
    ...
    return {
      ...
      speakers: [
        speaker('aisha', 'primary', 'precise continuity', `Current record: ${active}. Prior record: ${prior}.`, 'Anchoring'),
        speaker('claudia', 'side', 'dry practical', freshLine(body, [
          'Make it real: current value first, prior value visible, next design decision uses the current one.',
          'Make it real: compare current value, prior value, then decide from the current one. No rewrite needed.',
          'Structure it: current value first, prior value visible, next design decision uses the current one.'
        ]), 'Tracking next steps')
      ],
  ...
  if (active && prior) {
    return {
      ...
      speakers: [
        ...
        speaker('claudia', 'side', 'dry practical', freshLine(body, [
          'Make it real: use the current value going forward; keep the old value only for comparison.',
          'Make it real: current value drives the next move; old value stays as comparison.',
          'Make it real: old value answers the archive, current value guides the work.'
        ]), 'Tracking next steps')
      ],
      silentReactions: [reaction('vanya', 'Reading the room'), reaction('grok', 'Tracking')],
...
function continuityClaimFallback(text = '', body = {}) {
  ...
  if (isUpdate) {
    return {
      ...
      speakers: [
        ...
        speaker('claudia', 'side', 'dry practical', freshLine(body, [
          'Structure it: design from the current value; keep the prior value beside it only for comparison.',
          'Make it real: current value first, prior value visible, next design decision uses the current one.',
          'Make it real: compare current value, prior value, then decide from the current one. No rewrite needed.'
        ]), 'Tracking next steps')
      ],
  ...
  return {
    ...
    speakers: [
      ...
      speaker('claudia', 'side', 'dry practical', 'Make it real: if this changes, compare old and new before designing more. No quiet erasure.', 'Tracking next steps')
    ],
    silentReactions: [reaction('vanya', 'Reading the room'), reaction('grok', 'Tracking')],
...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (currentTopic(text) === 'movie_watch' || currentTopic(text) === 'watch_next') {
    if (recentWatchRecommendationShape(body) === 'first') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'host with taste', 'Room temperature first: pressure gets Heat, wonder gets Everything Everywhere All at Once, comfort with teeth gets Knives Out.', 'Reading the room'),
  ...
  if (currentTopic(text) === 'room_tension') {
    return {
      ...
      responseMode: 'open_floor',
      speakers: [
        speaker('vanya', 'primary', 'host with bite', 'Room temperature: useful wants to be alive, but alive keeps trying to become a help desk. That is the wobble.', 'Reading the room'),
  ...
  if (isFrustratedRecovery(text, body)) {
    ...
    if (recoveryShape === 'third') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Fair. Room temperature says the training answer is now a timer, not another explanation. Start the clock.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'One clean move: ten minutes, write one number down, stop. Repeat tomorrow before adding anything.', 'Tracking next steps'),
    ...
    if (recoveryShape === 'second') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Temperature check: the room is done negotiating. One workout, one meal, one sleep window; that is today.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'Make it real: push, pull, legs; log reps, recover, repeat. Leave two reps in reserve.', 'Tracking next steps'),
    ...
    if (recoveryShape === 'first') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Room energy, not a task queue: tiny vanity, massive discipline. Three repeatable training days; no heroic rebrand.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'Calendar first: Monday, Wednesday, Friday. Two rounds of push, row, squat, hinge; write reps down before you leave.', 'Tracking next steps'),
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    if (recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Tiny vanity gets twenty minutes. Stop negotiating with the clock: two minutes warm, sixteen minutes work, two minutes notes.', 'Reading the room'),
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Tiny vanity, clean discipline. Twenty minutes of training, small enough to finish and real enough that you feel it tomorrow.', 'Reading the room'),
  ...
  if (/\b(hungry|food|eat|eating|lunch|dinner|snack|meal|nutrition)\b/i.test(text)) {
    ...
    if (trainingFood) {
      return {
        ...
        speakers: [
          ...
          speaker('claudia', 'side', 'practical', 'Make it real before training: choose one timing rule: under an hour, banana and yoghurt; two hours, eggs and toast or rice and chicken.', 'Tracking next steps'),
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Room temperature: simple food, no drama. Eat enough to move, then let the afternoon stay human.', 'Reading the room'),
        speaker('claudia', 'side', 'practical', 'Make it real for lunch: rice and chicken, eggs and toast, a solid sandwich, or leftovers with water. Choose the one you can actually make.', 'Tracking next steps'),
  ...
  if (/sad|waste of time|stressed|tired|hurt|lonely/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm specific', 'Fair. Human temperature first: water, one clear surface, then the smallest next task. No grand reset.', 'Protective'),
        speaker('claudia', 'side', 'grounded practical', 'Set a timer. When it ends, write what changed: reps done, file opened, or one decision made.', 'Tracking next steps')
      ],
    ...
    return {
      ...
      speakers: [
        speaker('vanya', 'primary', 'warm reset', wantsToday
          ? 'Plain version: do one useful block today. Twenty minutes, one result, then stop before the room turns it into a thesis.'
          : 'Fair catch. Room temperature says the loop got loud; I am cutting the speeches and letting one useful move land.', 'Reading the room'),
        speaker('claudia', 'side', 'dry practical', wantsToday
          ? 'Make it real today: choose training or work, set twenty minutes, finish one visible result, and write the proof down.'
          : 'Make it real: next answer gets one shape, one useful move, and no recycled opener.', 'Tracking next steps'),
        speaker('grok', 'closer', 'dry diagnostic', wantsToday
          ...
          : 'Correct. The pattern repeated; the dodge has been identified.', 'Tracking failure')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('leah', 'Watching')],
  ...
  if (/logo|bland|taste|campaign|creative|weak idea/i.test(text)) {
    return {
      ...
      speakers: [
        ...
        speaker('claudia', 'side', 'dry practical', 'Make it real: simplify the shape, tighten spacing, and give the red accent one job: pulse, cut, or warning. Pick one.', 'Tracking next steps'),
        speaker('vanya', 'closer', 'social landing', 'Room temperature: give the mark quiet around it. Let people feel the signal before the brand starts explaining itself.', 'Reading')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('claudia', 'Tracking next steps')],
  ...
  if (/\b(content calendar|design client|plan tomorrow|planning tomorrow|help planning|campaign plan|schedule tomorrow|tomorrow)\b/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('claudia', 'primary', 'dry practical', 'Make it real for tomorrow: first decision, main build, cleanup. Put the riskiest block first and write the handoff before lunch.', 'Tracking next steps'),
