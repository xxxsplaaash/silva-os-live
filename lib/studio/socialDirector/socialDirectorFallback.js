    ...
    if (currentTopic(text) === 'watch_next' || recentWatchRecommendationShape(body) === 'first') {
      return {
        ...
        speakers: [
          ...
          speaker('vanya', 'side', 'host with taste', 'Keep it human: one title now; the room earns another sentence after the opening scene.', 'Reading the room')
        ],
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    ...
    if (referencedCompression && recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'single',
        speakers: [
          speaker('claudia', 'primary', 'practical', 'Run two rounds: squat, push, row, hinge, plank. Stop at twenty minutes and write the count.', 'Tracking next steps')
        ],
        silentReactions: [
