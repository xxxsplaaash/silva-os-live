  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    if (recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Leave the ceremony outside. One round first; the ego can decorate after the buzzer.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'Run the clock in quarters: legs, push, pull, core. One clean note at the buzzer, then stop.', 'Tracking next steps')
        ],
