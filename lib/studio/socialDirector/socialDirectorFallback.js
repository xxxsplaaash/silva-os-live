...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (isFrustratedRecovery(text, body)) {
    ...
    if (recoveryShape === 'second') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Good. Read the room: pressure before it becomes noise. Timer on, reps logged, same slot again this week.', 'Reading the room'),
    ...
    if (recoveryShape === 'first') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Keep it small enough to finish and human enough to repeat. Three training days, not a personality transplant.', 'Reading the room'),
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Twenty minutes, small enough to finish and real enough that tomorrow notices. Start the clock before the ego decorates it.', 'Reading the room'),
