...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Same twenty minutes, cleaner shape: one short training session, no heroic rebrand, leave while the body still trusts it.', 'Reading the room'),
