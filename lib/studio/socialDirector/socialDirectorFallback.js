...
function currentTopic(text = '') {
  ...
  if (/\b(what was my|what did i used to want|what did i use to want|what did i previously want|what did i earlier want)\b.*\b(old|original|earlier|previous|prior|used to)?\s*(preference|style|color|dashboard|landing page|brand)\b/i.test(text)) return 'continuity_change';
...
function directFallback(target, message = '') {
  ...
  if (/\b(sound fake|sounded fake|was that useful|useful or fake|be honest)\b/i.test(message)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('grok', 'primary', 'dry honest', 'Partly useful. Then it got too abstract and lost the person asking.', 'Tracking failure'),
...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (/\b(20 minutes|twenty minutes|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    return {
      ...
      speakers: [
        ...
        speaker('grok', 'closer', 'dry diagnostic', 'The constraint is useful. It forces a session you can actually finish.', 'Tracking')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('leah', 'Watching')],
  ...
  if (/sad|waste of time|stressed|tired|hurt|lonely/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm specific', 'Fair. If this feels dumb and stressful, pause the room and take one clean next move.', 'Protective'),
