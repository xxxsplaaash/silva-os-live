...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (isFrustratedRecovery(text, body)) {
    ...
    if (recoveryShape === 'first') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Start here: three training days this week, short enough that you actually repeat them.', 'Reading the room'),
  ...
  if (/sad|waste of time|stressed|tired|hurt|lonely/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm specific', 'Fair. No more room-theatre: do one small useful thing for ten minutes, then stop and check the result.', 'Protective'),
        speaker('claudia', 'side', 'grounded practical', 'Today that means one block, one visible output, no essay about the process.', 'Tracking next steps')
      ],
    ...
    return {
      ...
      speakers: [
        ...
        speaker('claudia', 'side', 'dry practical', wantsToday
          ? 'If it is training, run the short session. If it is work, open the first file and make one visible change.'
