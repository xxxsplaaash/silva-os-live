  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    if (recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Plain version: keep it breathable, start before your ego starts decorating the clock.', 'Reading the room'),
    ...
    return {
      ...
      speakers: [
        speaker('vanya', 'primary', 'warm reset', wantsToday
          ? 'Plain version: do one useful block today. Twenty minutes, one result, then leave the ceremony outside.'
