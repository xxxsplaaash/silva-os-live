...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (currentTopic(text) === 'movie_watch' || currentTopic(text) === 'watch_next') {
    if (recentWatchRecommendationShape(body) === 'first') {
      return {
        ...
        speakers: [
          ...
          speaker('leah', 'side', 'sharp taste', 'Skip consensus. Choose the film with teeth, not the one everyone can tolerate.', 'Holding critique'),
    ...
    return {
      ...
      speakers: [
        ...
        speaker('leah', 'side', 'sharp taste', 'One strong world, not wallpaper. Pick the feeling first; the title just admits the mood.', 'Holding critique'),
  ...
  if (isFrustratedRecovery(text, body)) {
    ...
    if (recoveryShape === 'third') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Fair. Tiny vanity, massive discipline. Start the clock before the ego decorates it.', 'Reading the room'),
    ...
    if (recoveryShape === 'second') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Fair. Tiny vanity gets today in three receipts: one workout, one meal, one sleep window.', 'Reading the room'),
    ...
    if (recoveryShape === 'first') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm reset', 'Fair. Tiny vanity, massive discipline. Three training days, no heroic rebrand.', 'Reading the room'),
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    if (recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Plain version: keep it breathable, start before the room turns it into a thesis.', 'Reading the room'),
          speaker('claudia', 'side', 'practical', 'Four stations: chair squat, incline push-up, backpack row, dead bug. Thirty seconds each, four rounds if the timer allows. Beat one number next time.', 'Tracking next steps')
        ],
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Tiny vanity gets twenty minutes on the clock; no heroic rebrand, no ego trying to decorate it. Small enough to finish, real enough that you can feel it tomorrow.', 'Reading the room'),
  ...
  if (/\b(hungry|food|eat|eating|lunch|dinner|snack|meal|nutrition)\b/i.test(text)) {
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Simple food, no drama. Eat enough to move, then let the afternoon stay human.', 'Reading the room'),
  ...
  if (/sad|waste of time|stressed|tired|hurt|lonely/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm specific', 'Fair. Body first: water, one clear surface, then the smallest next task. No grand reset.', 'Protective'),
    ...
    return {
      ...
      speakers: [
        speaker('vanya', 'primary', 'warm reset', wantsToday
          ...
          : 'Fair catch. Tiny vanity hates recycled speeches. One useful move lands now; the rest can wait.', 'Reading the room'),
  ...
  if (/logo|bland|taste|campaign|creative|weak idea/i.test(text)) {
    return {
      ...
      speakers: [
        ...
        speaker('vanya', 'closer', 'social landing', 'Give the mark quiet around it. Let people feel the signal before the brand starts explaining itself.', 'Reading')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('claudia', 'Tracking next steps')],
  ...
  if (/\b(content calendar|design client|plan tomorrow|planning tomorrow|help planning|campaign plan|schedule tomorrow|tomorrow)\b/i.test(text)) {
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('claudia', 'primary', 'dry practical', 'Make it real for tomorrow: pick the three things that would make the day count, put the hardest one first, and leave a buffer.', 'Tracking next steps'),
