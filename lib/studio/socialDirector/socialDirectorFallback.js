...
function isFrustratedRecovery(text = '', body = {}) {
  ...
  return /\b(bruh|bro|wtf|what is the objective|where do i start)\b/i.test(text)
    && /\b(objective is clear|not discussing|personal fitness|focus is required|muscle|muscles|fitness|workout|training|push-ups|pushups|squats?|split squats?|hinges?|planks?|rows|core|sessions?|push,?\s+pull,?\s+legs|log reps|recover,?\s+repeat|proof talk after|no slogan)\b/i.test(joined);
}

function recentFitnessRecoveryShape(body = {}) {
  const recent = recentAssistantText(body);
  if (/\b(action version:\s*push,?\s+pull,?\s+legs|no slogan|proof talk after|leave two reps in reserve)\b/i.test(recent)) return 'third';
  ...
  if (recentShortSessionShape(body)) return 'first';
...
function recentShortSessionShape(body = {}) {
  const recent = recentAssistantText(body);
  if (/\b(twenty minutes is enough|twenty minutes of training|twenty minutes is a good target|20 minutes is a good target|set a timer for twenty minutes|focused session|rest periods short|three rounds: squat or hinge|do three rounds|focus on three rounds|three rounds of incline push-ups|run two rounds|two rounds of push, pull, hinge|push, pull, hinge, and core|warm up for three|3 minutes warm-up|record total reps|write the (rep )?count|stop at twenty minutes|wall push-ups|glute bridges|5 minutes for plank|forty seconds on, twenty off|45 seconds on|15 seconds rest|repeat the circuit three times|one small circuit|clock honest|clock do the arguing)\b/i.test(recent)) return 'first';
  return '';
}

function isReferencedCompressionRequest(text = '') {
  return /\b(turn|make|convert|compress)\b.{0,50}\b(that|this|it|referenced)\b/i.test(text);
}
...
function socialFallbackFor(message = {}, body = {}) {
  ...
  if (currentTopic(text) === 'movie_watch' || currentTopic(text) === 'watch_next') {
    if (currentTopic(text) === 'watch_next' || recentWatchRecommendationShape(body) === 'first') {
      return {
        roomBeat: 'The room continues the watch choice instead of replaying the same titles.',
        roomMood: 'playful',
        responseMode: 'small_exchange',
        speakers: [
          speaker('leah', 'primary', 'sharp taste', 'Taste call: Heat for pressure, Knives Out for social teeth, Everything Everywhere All at Once for bright chaos.', 'Holding critique'),
          speaker('vanya', 'side', 'host with taste', 'Keep it human: press play, let the night breathe, and save the debate for credits.', 'Reading the room')
        ],
        silentReactions: [reaction('aisha', 'Anchoring'), reaction('claudia', 'Tracking next steps'), reaction('grok', 'Tracking')],
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('leah', 'primary', 'sharp taste', 'Arrival for quiet pressure, Spider-Verse for voltage, The Menu if you want bite.', 'Holding critique'),
        speaker('vanya', 'side', 'host with taste', 'Keep the night human: one title now, then the room earns another sentence after the first scene.', 'Reading the room')
      ],
      silentReactions: [reaction('aisha', 'Anchoring'), reaction('claudia', 'Tracking next steps'), reaction('grok', 'Tracking')],
  ...
  if (isFrustratedRecovery(text, body)) {
    ...
    if (recoveryShape === 'third') {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('claudia', 'primary', 'practical', 'One clean training move: ten minutes, write reps down, stop. Repeat tomorrow before adding anything.', 'Tracking next steps'),
          speaker('vanya', 'side', 'warm reset', 'Fair catch. No task badge; first rep gets the floor now.', 'Reading the room')
        ],
    ...
    if (recoveryShape === 'second') {
      return {
        ...
        speakers: [
          ...
          speaker('grok', 'closer', 'dry diagnostic', 'Premise check: specific beats motivational fog. Start before the speech writes itself.', 'Tracking')
        ],
  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    const referencedCompression = isReferencedCompressionRequest(text);
    if (referencedCompression && recentShortSessionShape(body)) {
      return {
        roomBeat: 'The room compresses the referenced workout without repeating the prior short-session shape.',
        roomMood: 'focused',
        responseMode: 'single',
        speakers: [
          speaker('claudia', 'primary', 'practical', 'Run two rounds: squat, push, row, hinge, plank. Stop at twenty minutes and write the rep count.', 'Tracking next steps')
        ],
        silentReactions: [
          reaction('vanya', 'Reading the room', 'letting the practical plan land without adding more heat'),
          reaction('aisha', 'Anchoring'),
          reaction('leah', 'Watching'),
          reaction('grok', 'Tracking')
        ],
        stateUpdates: { notes: ['Referenced short-session compression varied; no durable memory fact created.'] }
      };
    }
    if (!referencedCompression && recentShortSessionShape(body)) {
      return {
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('claudia', 'primary', 'practical', 'Twenty minutes: 3 to warm up, 12 for squats, wall push-ups, towel rows, and glute bridges, 5 for plank. Record total reps.', 'Tracking next steps'),
        speaker('vanya', 'side', 'warm practical', recentShortSessionShape(body)
          ? freshLine(body, [
            'Small enough to finish; rude enough to count.',
            'The glamour can wait. Finish the set and leave evidence.',
            'Make it socially impossible to negotiate: twenty minutes, then proof.'
          ])
          : freshLine(body, [
            'Make it socially impossible to negotiate: twenty minutes, then proof.',
            'Small enough to finish; rude enough to count.',
