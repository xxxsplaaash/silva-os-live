  ...
  if (/\b(20 minutes?|twenty minutes?|only have \d+\s*minutes|limited time)\b/i.test(text) && hasFitnessContext(text, body)) {
    if (recentShortSessionShape(body)) {
      return {
        ...
        responseMode: 'small_exchange',
        speakers: [
          speaker('vanya', 'primary', 'warm practical', 'Same twenty minutes, cleaner shape. Set the timer, move through four stations, and let the clock do the arguing.', 'Reading the room'),
    ...
    return {
      ...
      responseMode: 'small_exchange',
      speakers: [
        speaker('vanya', 'primary', 'warm practical', 'Set twenty minutes. First round proves the day is moving; second round earns the confidence.', 'Reading the room'),
    ...
    return {
      ...
      speakers: [
        speaker('vanya', 'primary', 'warm reset', wantsToday
          ...
          : 'I am keeping the room human and cutting the loop. One useful move lands now.', 'Reading the room'),
