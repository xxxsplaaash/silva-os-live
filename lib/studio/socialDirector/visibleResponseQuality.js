...
function hasSelfTheater(text = '') {
  return /\b(mistaking polish for progress|actual work not the presentation|stuck between wanting to be useful and sounding like it|the room is stuck between|perform usefulness|perform the process of being useful|loop of self critique|room is stuck in a loop|evidence of completion is the only metric|capability was present|taste under pressure|variable that needs calibration|move to the next item|objective is energy|objective is entertainment|parameters were clear|within those parameters|operational parameters|operational reality|design parameters|parameters updated|parameters are updated|adjust the configuration|adjust the design parameters|adjust the temperature|temperature later|map the accent placement|key interactive elements|proceed with white editorial|clear enough for implementation|configuration is now active|configuration is now the standard|system configuration|update the system configuration|updated the system|system will reflect|system to reflect|corrected dashboard settings|dashboard settings|active setting|active selection|loads fast|converts|build supports|performance lag|visual intensity|current standard)\b/.test(text);
}
...
function hasSocialCheckInDrift(text = '', topic = '') {
...
function evaluateVisibleResponse({
  ...
  if (hasSocialCheckInDrift(text, topic)) issues.push(issue('stale-context', 'social-check-in'));
