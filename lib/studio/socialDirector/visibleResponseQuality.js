...
const BLIND_ATTRIBUTION_RULES = Object.freeze({
  ...
  vanya: [
    [/\b(tiny vanity|massive discipline|we can work with that|room turns it into a thesis|task badge|task queue|turning into a queue|turning this into a task queue|plain version|no heroic rebrand|heroic rebrand|temperature|change the temperature|irritation noted|breathable|breathe|cool|dramatic|drama|alive|restless|human|fair|socially|underneath)\b/, 4],
    [/\b(open the room|read(?:ing)? the room|room energy|pressure before it becomes noise|without turning it into therapy)\b/, 3],
    [/\b(room|warm|playful|dignity|ten minutes|water|clear one surface)\b/, 1]
  ],
  leah: [
    [/\b(bland|boredom|boring|taste|status|committee|consensus|edge|side eye|suspicious|too neat|safe choice|pressure test|sharp|stylish|pick the feeling|title is just|mood it wants|quiet pressure|voltage|room wants bite|wants bite|social teeth|spectacle|black glass|one red pulse|quiet hero|generic saas|hero mood|decorative compromise|protect the restraint|warning light|wellness badge|girl i am here|acting surprised)\b/, 4],
  ...
  claudia: [
    [/\b(start with|first step|same days|one clean move|next move|one blunt next move|lower the noise|one decision|change the pattern|one concrete action|choose one task|short timer|rough pass|structure|owned|owner|cut it down|write (one|the) number down|write (the )?reps down|same twenty minutes|twenty minutes of training|warm up|run the clock|make it real|incline push|backpack row|split squat|hip hinge|squat or hinge|two rounds|mark completion|track the work|recover,? repeat|boring baseline|small progression|planning session|before training|eat light enough|start repeatable|repeatable|adjusted next time)\b/, 4],
    [/\b(push|push ups|pushups|incline push|squat|split squat|hinge|hip hinge|row|backpack row|plank|rounds?|session|schedule|block|result|clear space|practical|usable|current record first|log reps?|logged|add one clean rep|before training|eat light enough|rice and chicken|eggs and toast)\b/, 2],
    [/\b(start|steps?|plan|today|week|minutes?|do this|first|second|third)\b/, 1]
  ],
  grok: [
    [/\b(narrative ambition|premise|fault line|fault|dodge|fake part|partly useful|weak point|track reps|evidence beats|sloppy|contradiction|objection|five adjectives|shape is unemployed|not volunteering|jazz hands)\b/, 4],
...
function blindAttributionForLine(value = '') {
  ...
  const identifiable = wordTotal < 7 || (top.score >= 4 && confidence >= 2);
  return {
...
function evaluateBlindAttributionLines(lines = []) {
  ...
  const results = source
    .map(item => {
      ...
      const issueCategory = attribution.wordTotal >= 7 && !attribution.identifiable
        ? 'blind-attribution'
        : '';
      return {
        expectedSpeakerId,
