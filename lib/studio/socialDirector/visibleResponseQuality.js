...
function hasGenericAdvice(text = '') {
  const markers = [
    ...
    /\b(lean protein|complex carbs|complex carbohydrates|turkey sandwich|whole wheat|protein bar|apple can hold you over)\b/,
...
function weakNormalAnswer(text = '', userMessage = '') {
  ...
  if (/\b(what is the one thing you need to do next|what part of the last answer|room needs a clear ask|state the actual problem|identify the core requirement|current exchange is not yielding|clear ask to move forward|focus on one concrete action for today|single most important task|what is the single most important task|finalize the q3 brief|q3 brief|no other objectives|what is it)\b/.test(text)) return true;
