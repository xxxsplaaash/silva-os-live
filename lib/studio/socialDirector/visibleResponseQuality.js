...
function hasFlatSocialRollCall(text = '', topic = '', userMessage = '') {
  if (topic !== 'social') return false;
  if (!/\b(how is everyone|how are you all|how is the room|how are we|hows everyone|hows the team|everyone ok|everyone okay)\b/.test(normalizeText(userMessage))) return false;
  if (/\b(focused on the current objective|current objective|current state|all systems nominal|current episode parameters|current episode is stable|episode is stable|human temperature is stable|aesthetic standards are holding|no blandness detected|operational flow is clear|next steps are defined|no immediate faults detected|monitoring for anomalies|emergent anomalies|operational ready|ready for the next step|present and ready to engage|signal is clear|structured and awaiting direction|awaiting direction|functioning though|how implies a metric|observing the room|observing current state|operational status)\b/.test(text)) return true;
...
function repetitionComplaintDeflected(text = '', userMessage = '') {
  ...
  if (/\b(what is the one thing we need to land here|what is the one thing we need to land|define the ask clearly|who owns the next step|let s name the ask|name the ask clearly|state the actual problem|what is the actual problem|what is the one thing you need to do next|identify the core requirement|current exchange is not yielding)\b/.test(text)) return true;
