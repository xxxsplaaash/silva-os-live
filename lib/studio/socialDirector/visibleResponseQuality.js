...
const BLIND_ATTRIBUTION_RULES = Object.freeze({
  ...
  vanya: [
    [/\b(same twenty minutes,? cleaner shape|body trusts it|ego tries to decorate|small enough to finish|real enough that you can feel|feel it tomorrow)\b/, 4],
    [/\b(same twenty minutes,? cleaner shape|tiny vanity|massive discipline|we can work with that|room turns it into a thesis|task badge|task queue|turning into a queue|turning this into a task queue|plain version|no heroic rebrand|heroic rebrand|cleaner shape|body trusts it|ego tries to decorate|small enough to finish|real enough that you can feel|feel it tomorrow|warmth in the room|drift past|temperature|change the temperature|irritation noted|breathable|breathe|cool|dramatic|drama|alive|restless|human|fair|socially|underneath)\b/, 4],
...
function evaluateBlindAttributionLines(lines = []) {
  ...
  const results = source
    .map(item => {
      ...
      let issueCategory = '';
      if (attribution.wordTotal >= 7 && !attribution.identifiable) {
