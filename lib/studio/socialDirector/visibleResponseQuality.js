...
function sameSubjectRecentContinuityClaim(userMessage = '', recentTurns = []) {
  const subject = continuityClaimSubject(userMessage);
  if (!subject) return false;
  return (Array.isArray(recentTurns) ? recentTurns : [])
    .filter(item => /^user$/i.test(String(item?.speakerId || item?.role || '')))
    .map(item => String(item?.text || item?.content || ''))
    .some(text => continuityClaimSubject(text) === subject);
}
...
function hasCrossDomainPriorReceipt(text = '', userMessage = '', recentTurns = [], continuity = {}) {
  const current = normalizeText(userMessage);
  if (/\b(what changed|what was changed|what did .* change|difference|previous|superseded|old|original|earlier|used to|never said|did i say|did i ever say)\b/.test(current)) return false;
  const subject = continuityClaimSubject(userMessage);
  if (!subject) return false;
  if (!/\bprior record|previous record|old record\b/.test(text)) return false;

  const priorIndex = text.search(/\b(prior|previous|old) record\b/);
  const currentAfterPrior = text.slice(Math.max(0, priorIndex + 1)).search(/\bcurrent record\b/);
  const priorSegment = currentAfterPrior >= 0
    ? text.slice(priorIndex, priorIndex + 1 + currentAfterPrior)
    : text.slice(priorIndex);
  const priorSubject = continuityClaimSubject(priorSegment);
  if (priorSubject && priorSubject !== subject) return true;

  const hasPriorEvidence = Number(continuity.superseded || 0) > 0
    || Number(continuity.disputed || 0) > 0
    || continuityTexts(continuity, ['supersededTexts', 'supersededTruthTexts', 'priorTexts']).length > 0;
  if (hasPriorEvidence) return false;

  const isExplicitSupersession = /\b(actually|changed|now|instead)\b/.test(current);
  if (isExplicitSupersession && sameSubjectRecentContinuityClaim(userMessage, recentTurns)) return false;
...
function hasRoomPerformanceLanguage(text = '') {
  return /\b(room theatre|theatrical planning|stop performing|customer support|polished dodge|mostly fake sounding|fake sounding|less doctrine|performance art|sustenance is a parameter|is a parameter not|execute|evidence beats another round|failed answer wearing a badge|perform a job title|circling the drain|not landing anywhere concrete|stress is the signal|stress is a symptom of the dodge|room needs to take a position|observe the tension|room will carry the continuity|continuity status and contradictions surface|as the exchange lands|public room is live|room is live|the room is holding|room holds this turn|room is processing|social simulation|multi agent room|characters respond dynamically|pack 1 connected|postgres connected|keep(?:ing)? the room human|cutting the loop|one useful move lands now|stop narrating the plan|dodge has been identified)\b/.test(text);
}
...
function thinMovieChoice(text = '', userMessage = '') {
  if (currentTopic(userMessage) !== 'movie') return false;
  return /\b(pick the feeling first|title just admits the mood|choose the film with teeth|not the one everyone can tolerate|specific title matters less than the mood|movie is just the vibe)\b/.test(text);
}
...
function qualityCheckExampleLeak(text = '', userMessage = '') {
  const current = normalizeText(userMessage);
  if (/\b(useful or fake|sound fake|sounded fake|be honest)\b/.test(current)) return false;
  return /\b(partly useful|useful (half|part)|fake (half|part)|it named the dodge|stopped answering the person|abstract and stopped answering|became critique instead of answer)\b/.test(text);
}
...
function weakNormalAnswer(text = '', userMessage = '') {
  ...
  if (/\b(assign one owner to the handoff|owner to the handoff|break the day into three blocks|hardest task.{0,90}cleanup.{0,90}buffer|cleanup block.{0,90}breathable gap|one hard thing early.{0,90}cleanup block)\b/.test(text)) return true;
...
function fitnessContextIgnored(text = '', userMessage = '', recentTurns = []) {
  const current = normalizeText(userMessage);
  if (!/\b(bruh|bro|wtf|where do i start|what is the objective|ok but i only have 20 minutes|only have 20 minutes)\b/.test(current)) return false;
  const recent = normalizeText(recentAssistantText(recentTurns));
  if (!/\b(muscle|muscles|fitness|workout|training|train|push ups|pushups|rows|squats|hinges|plank|reps|rounds|sessions)\b/.test(recent)) return false;
  if (/\b(rough pass|visible result|switching tasks|make the day smaller|one useful pass|less ceremony|enough air to stay human)\b/.test(text)
    && !/\b(push|pull|row|squat|hinge|plank|lunge|core|reps|rounds|training day|three days|monday|wednesday|friday|first set|rep count)\b/.test(text)) {
    return true;
  }
  return !/\b(push|pull|row|squat|hinge|plank|lunge|core|reps|rounds|timer|twenty minutes|20 minutes|three days|monday|wednesday|friday|first set|rep count)\b/.test(text);
}

function bruhReplaysObjectiveRecovery(text = '', userMessage = '') {
  const current = normalizeText(userMessage);
  if (!/\b(bruh|bro|wtf)\b/.test(current)) return false;
  return /\b(no slogan|proof talk after|action version|push pull legs|log reps recover repeat|leave two reps in reserve)\b/.test(text)
    && /\b(push|pull|legs|reps|ten minutes|proof|reserve)\b/.test(text);
}
...
function frustrationMiss(text = '', userMessage = '') {
  ...
  if (/\b(human signal|connection is off|words (do not|dont) land|words (are not|arent) landing|current record|prior record|room is abstract|room was direct)\b/.test(text)) return true;
...
function evaluateVisibleResponse({
  ...
  if (hasCrossDomainPriorReceipt(text, userMessage, recentTurns, continuity)) issues.push(issue('continuity-conflict', 'cross-domain-prior'));
  ...
  if (fitnessContextIgnored(text, userMessage, recentTurns)) issues.push(issue('topic-ignored', 'fitness-context'));
  if (bruhReplaysObjectiveRecovery(text, userMessage)) issues.push(issue('frustration-miss', 'objective-replay'));
  ...
  if (thinMovieChoice(text, userMessage)) issues.push(issue('speaker-flatness', 'thin-movie-choice'));
  if (thinQualityJudgment(text, userMessage)) issues.push(issue('speaker-flatness', 'thin-quality-judgment'));
  if (qualityCheckExampleLeak(text, userMessage)) issues.push(issue('example-leak', 'quality-check'));
