...
function hasGenericAdvice(text = '') {
  const markers = [
    ...
    /\b(thats a solid goal|that s a solid goal|thats a great goal|that s a great goal|three days a week is a solid start|fuel yourself well|enough sleep to support|support that growth|muscles huh|lets get you started|no gym no problem|miracles overnight)\b/
  ];
...
function hasGenericWarmth(text = '') {
  return /\b(that s a fair reaction|thats a fair reaction|fair reaction|that is a fair reaction|valid reaction|that s a valid reaction|thats a valid reaction|that is a valid reaction|really valid|your feelings are valid|i hear you|i appreciate you sharing|holding space|safe space)\b/.test(text);
}
