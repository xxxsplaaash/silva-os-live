...
const EXCHANGE_SCHEMA_VERSION = 'studio-pulse.exchange.v0.6';
...
function normalizeExchangeStateV06(input = {}) {
  const next = input && typeof input === 'object' ? input : {};
  return {
    schemaVersion: EXCHANGE_SCHEMA_VERSION,
    addendumCooldownTurns: clampInt(next.addendumCooldownTurns, 0, 0, 3),
    aishaCloseCooldownTurns: clampInt(next.aishaCloseCooldownTurns, 0, 0, 4),
    lastExchangeMode: String(next.lastExchangeMode || ''),
    lastAddendumSpeakerId: String(next.lastAddendumSpeakerId || ''),
    lastCommandCloseAtTurn: clampInt(next.lastCommandCloseAtTurn, 0, 0, 999999),
    updatedAtTurn: clampInt(next.updatedAtTurn, 0, 0, 999999)
  };
}
...
function normalizeCharacterContinuityState(input = {}, options = {}) {
  ...
  return {
    ...
    exchangeStateV06: normalizeExchangeStateV06(prev.exchangeStateV06),
...
function reduceCharacterContinuityState({
  ...
  const previousExchange = normalizeExchangeStateV06(next.exchangeStateV06);
  const plannedSteps = Array.isArray(plan.steps) ? plan.steps : [];
  const visibleStepRoles = new Set(
    visibleTurns.map(turn => String(turn?.exchangeRole || '').trim())
      .filter(Boolean)
  );
  const plannedAddendumSpeaker = plannedSteps.find(step => String(step?.exchangeRole || '') === 'addendum')?.speakerId || '';
  const commandCloseVisible = visibleStepRoles.has('command-close')
    || visibleTurns.some(turn => String(turn?.responseIntent || '').trim() === 'command-close');
  const addendumVisible = visibleStepRoles.has('addendum')
    || visibleTurns.some(turn => String(turn?.responseIntent || '').trim().includes('addendum'));
  next.exchangeStateV06 = normalizeExchangeStateV06({
    ...previousExchange,
    addendumCooldownTurns: addendumVisible ? 1 : Math.max(0, previousExchange.addendumCooldownTurns - 1),
    aishaCloseCooldownTurns: commandCloseVisible ? 2 : Math.max(0, previousExchange.aishaCloseCooldownTurns - 1),
    lastExchangeMode: String(plan.exchangeMode || previousExchange.lastExchangeMode || ''),
    lastAddendumSpeakerId: String(addendumVisible ? (plannedAddendumSpeaker || previousExchange.lastAddendumSpeakerId) : previousExchange.lastAddendumSpeakerId || ''),
    lastCommandCloseAtTurn: commandCloseVisible ? next.expressiveHabitatV0.updatedAtTurn : previousExchange.lastCommandCloseAtTurn,
    updatedAtTurn: next.expressiveHabitatV0.updatedAtTurn
  });
  next.updatedAt = now;
...
function expressiveHabitatContextForAisha(continuityState = {}, options = {}) {
  ...
  const exchange = options.exchangeContext && typeof options.exchangeContext === 'object' ? options.exchangeContext : {};
  const exchangeRole = String(exchange.exchangeRole || '').trim();
  const exchangeMode = String(exchange.exchangeMode || plan.exchangeMode || '').trim();
  return {
    schemaVersion: EXPRESSIVE_SCHEMA_VERSION,
    ...
    exchangeMode: exchangeMode || 'solo',
    exchangeRole: exchangeRole || 'primary',
    addendumSpeakerId: String(exchange.addendumSpeakerId || plan.addendumSpeakerId || '').trim(),
    commandCloseSpeakerId: String(exchange.commandCloseSpeakerId || plan.commandCloseSpeakerId || '').trim(),
    sideCommentAllowed: exchangeRole === 'addendum',
    speakingConstraints: [
      exchangeRole === 'addendum'
        ? 'Write one short side note for the already-planned addendum speaker.'
        : exchangeRole === 'command-close'
          ? 'Write one concise command close; do not reopen the floor.'
