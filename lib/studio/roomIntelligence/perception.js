...
const STRICT_PRESENCE_RX = /\b(where is everyone|where is everybody|where is everyone else|who(?:'s| is) here|who(?:'s| is) online|who(?:'s| is) around|who else is here|is anyone here|room state|presence)\b/i;
const ROLL_CALL_RX = /\b(?:role call|roll call|sound off|everyone check in|who(?:'s| is) here|who(?:'s| is) around|who else is here|where is everyone|where is everybody|where is everyone else)\b|^\s*everyone else\s*[?!.]*\s*$/i;
