...
function isResolvedHighestValueFollowUp(perception = {}) {
  const text = originalTurnText(perception);
  const previous = String(perception.previousQuestion || '').trim();
  const context = [previous, perception.text, perception.topicFocus].map(item => String(item || '')).join(' ');
  return perception.resolvedFromHistory === true
...
function planRoomTurnRaw({ perception = {}, roomState = {}, socialImpulses = [] } = {}) {
  ...
  if (isResolvedHighestValueFollowUp(perception)) {
