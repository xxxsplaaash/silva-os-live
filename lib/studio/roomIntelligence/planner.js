...
function pizzaText() {
  return "Pizza roll call? Vanya is obviously yes. Aisha allows it if the meeting has an actual point, Leah judges the topping choices, Claudia wants plates and timing sorted, and Grok is pretending not to care while taking the last slice.";
}
...
function planRoomTurn({ perception = {}, roomState = {}, socialImpulses = [] } = {}) {
  ...
  if (perception.taskType === 'social_food') {
    return {
      intentFamily: 'casual-social',
      deterministic: true,
      requiresProvider: false,
      responseOrder: ['vanya'],
      steps: [
        eventStep({
          speakerId: 'vanya',
          responseIntent: 'social-read',
          reason: 'casual-room-social-question',
          deterministicText: pizzaText(),
          tone: 'playful',
          socialImpulse: impulseFor(socialImpulses, 'vanya'),
          roomStateDelta: { roomMood: 'warm' }
        })
      ...
      socialImpulses,
      trace: 'casual social question answered by warm room read'
    };
  }
