...
function rollCallText(roomState = {}) {
  const groups = presenceGroups(roomState);
  ...
  return `Roll call: ${active} are present. ${quiet} are quiet/listening, not absent; they are available when called in.${away}${unknown}`;
}
...
function rollCallSteps(roomState = {}, perception = {}, socialImpulses = []) {
  ...
  return [
    eventStep({
      ...
      deterministicText: rollCallText(roomState),
      tone: 'focused',
