...
function groupOpinionCopy(id = '', perception = {}) {
  ...
  const genericCopy = {
    aisha: `On ${topic}: I want the clear purpose first. What is it meant to make someone feel, choose, or remember?`,
    leah: `On ${topic}: show me the actual thing and I will tell you if it has taste or just confidence.`,
    claudia: `On ${topic}: I would judge it by use case, audience, deadline, and whether it survives real delivery.`,
    grok: `On ${topic}: I need the artifact or constraints. Otherwise I am only judging fog with punctuation.`,
    vanya: `On ${topic}: I care how it lands with people. Send the piece or the context and I will give you the room read.`
  };
...
function planRoomTurn({ perception = {}, roomState = {}, socialImpulses = [] } = {}) {
  ...
  return {
    intentFamily: perception.taskType === 'technical' ? 'technical-diagnosis' : perception.taskType === 'creative' ? 'creative-room' : 'casual-room',
    deterministic: false,
    requiresProvider: true,
    responseOrder: [leadId],
    steps: [
      eventStep({
        speakerId: leadId,
        responseIntent: perception.requestedCharacterIds.length ? 'direct-answer' : 'room-answer',
