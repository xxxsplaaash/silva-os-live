...
function isFullRoomOnlineRollCall(perception = {}) {
  const text = originalTurnText(perception).toLowerCase();
  return perception.asksRollCall === true
    && /\b(everyone|everybody|all of you|all five|whole room)\b/.test(text)
    && /\b(come online|online|role call|roll call|sound off|check in)\b/.test(text);
}

function fullRoomOnlineRollCallSteps(roomState = {}, socialImpulses = []) {
  const copy = {
    aisha: 'Role call. Aisha here, holding the room.',
    vanya: 'Vanya here, reading the temperature.',
    leah: 'Leah here. Holding critique until there is something worth cutting into.',
    claudia: 'Claudia here. Tracking next steps and constraints.',
    grok: 'Grok here. Watching the failure patterns, regrettably.'
  };
  return ['aisha', 'vanya', 'leah', 'claudia', 'grok']
    .filter(id => speakerPresence(roomState, id) !== 'away')
    .map(id => eventStep({
      speakerId: id,
      responseIntent: 'available-roll-call',
      reason: 'user-asked-everyone-online-for-role-call',
      deterministicText: copy[id],
      ...
      exchangeRole: id === 'aisha' ? 'primary' : 'addendum',
      exchangeMode: 'solo-plus-addendum',
      maxBubbleLength: 140,
...
function rollCallSteps(roomState = {}, perception = {}, socialImpulses = []) {
  if (isFullRoomOnlineRollCall(perception)) return fullRoomOnlineRollCallSteps(roomState, socialImpulses);
...
function originalTurnText(perception = {}) {
  return String(perception.originalText || perception.text || '').replace(/\s+/g, ' ').trim();
}

function openFloorSubject(perception = {}) {
  const topic = String(perception.topicFocus || '').trim();
  if (topic && topic.toLowerCase() !== 'that') return topic;
  return originalTurnText(perception)
    .replace(/\b(?:open floor|open the floor|hear from the room|room perspective|whole room perspective|full room perspective|bring the room in|let the room weigh in|(?:team|room),?\s+weigh in)\b/ig, '')
    .replace(/\b(on|about|for|please|pls|team|room|everyone|everybody)\b/ig, ' ')
    .replace(/[?.!,;:\-]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

function hasOpenFloorSubject(perception = {}) {
  const topic = String(perception.topicFocus || '').trim();
  if (topic && topic.toLowerCase() !== 'that') return true;
  const subject = openFloorSubject(perception);
  if (!subject) return false;
  if (/^(this|that|it|the thing|the object|whatever|anything)$/i.test(subject)) return false;
  return subject.split(/\s+/).filter(Boolean).length >= 2 || /\b(logo|campaign|plan|bug|failure|timeout|provider|decision|direction|brief|copy|design|artifact)\b/i.test(subject);
}

function openFloorMissingContextSteps(socialImpulses = []) {
  return [eventStep({
    speakerId: 'vanya',
    responseIntent: 'open-floor-context-request',
    reason: 'explicit-open-floor-missing-subject',
    deterministicText: 'Open floor needs a subject, babe. Bring the logo, plan, bug, decision, or failure path, and then the room can split it properly.',
    tone: 'warm',
    socialImpulse: impulseFor(socialImpulses, 'vanya'),
    roomStateDelta: { roomMood: 'attentive' },
    exchangeRole: 'primary',
    exchangeMode: 'open-floor',
    exchangeGoal: 'ask for the missing subject before opening the room',
    maxBubbleLength: 220,
    deterministicRequired: true
  })];
}
...
function directFallbackSubject(perception = {}) {
  const topic = String(perception.topicFocus || '').trim();
  const raw = topic && topic !== 'that' ? topic : originalTurnText(perception);
  const cleaned = raw
...
function openFloorCopy(id = '', perception = {}) {
  const topic = topicLabel(perception);
  const copy = {
    vanya: `Room read: ${topic} needs one human landing and then only the specialists with real signal.`,
    leah: `Taste read: show the creative piece clearly and I will name where it gets brave or goes bland.`,
    claudia: `Delivery read: ${topic} needs an owner, a constraint, and a next move that can survive delivery.`,
    grok: `Diagnostic read: I am checking whether ${topic} is a real pattern, not just noise wearing punctuation.`,
...
function aishaCommandCloseText(perception = {}) {
  ...
  return topic && topic !== 'that'
    ? `Close it there: hold the useful signal for ${topic}, choose the next owner, and do not turn this into a chorus.`
    : 'Close it there: choose the subject first, then the room can add signal without becoming a chorus.';
}

function openFloorSteps(roomState = {}, perception = {}, socialImpulses = []) {
  if (!hasOpenFloorSubject(perception)) return openFloorMissingContextSteps(socialImpulses);
  const seen = new Set();
...
function directConflictStep(targetId = '') {
  const copy = {
    ...
    claudia: 'Useful starts with specifics: owner, step, deadline, blocker. Bring the useful signal and I will sort the rest.',
    grok: 'Useful starts when you give me the failure, not a verdict. What broke, what changed, and what evidence do we have?',
    vanya: 'The heat is obvious. Aim it at the broken part and I will stay with you, but I am not becoming the target.'
  };
  return eventStep({
    speakerId: targetId,
...
function ordinaryFallbackText(speakerId = '', perception = {}) {
  const text = originalTurnText(perception);
  ...
  if (/\b(nex\s*t|next).{0,24}\b(highest|valuable|value|move)\b|\b(highest|most).{0,24}\bvaluable.{0,24}\bmove\b/i.test(lower)) {
  ...
  if (/\b(hungry|lunch|dinner|snack|coffee|chit\s*chat|banter)\b/i.test(lower)) {
  ...
  if (/\bless stiff\b|\bmake this\b.*\bstiff\b|\bnot so stiff\b/i.test(lower)) {
  ...
  if (perception.taskType === 'social_preference') {
    const topic = topicLabel(perception);
    return `Room read: ${topic} is a social question, not a survey. I can give the warm read, and you can call in sharper takes if you want them.`;
  }
...
function isResolvedHighestValueFollowUp(perception = {}, roomState = {}) {
  const text = originalTurnText(perception);
  const previous = String(perception.previousQuestion || '').trim();
  const context = [previous, perception.text, perception.topicFocus, roomState.currentTopic]
    .map(item => String(item || ''))
    .join(' ');
  return (perception.resolvedFromHistory === true || /\b(highest|valuable|next move|next most valuable|move)\b/i.test(String(roomState.currentTopic || '')))
    && /^(what|what\?|huh|wait what)\s*[?.!]*$/i.test(text)
    && /\b(highest|valuable|next move|next most valuable|move)\b/i.test(context);
}
...
function planRoomTurnRaw({ perception = {}, roomState = {}, socialImpulses = [] } = {}) {
  ...
  if (isResolvedHighestValueFollowUp(perception, roomState)) {
    const leadId = chooseOrdinaryLead({ ...perception, asksRollCall: false, taskType: 'conversation' }, socialImpulses);
    return {
      ...
      steps: [
        eventStep({
          ...
          reason: 'resolved-highest-value-follow-up',
          deterministicText: ordinaryFallbackText(leadId, perception),
          tone: characterProfile(leadId).tone,
          socialImpulse: impulseFor(socialImpulses, leadId),
          roomStateDelta: { roomMood: 'focused' }
        })
      ],
      socialImpulses,
      trace: 'resolved terse highest-value follow-up beats stale roll-call state'
    };
  }
