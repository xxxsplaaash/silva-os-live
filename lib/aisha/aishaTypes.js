...
function normalizeRecentMessage(item = {}) {
  const timestamp = text(item.timestamp || item.createdAt || item.ts || '');
  return {
...
function normalizeCharacterStates(states = {}) {
  return Object.fromEntries(
    Object.entries(plainObject(states)).map(([id, state]) => [text(id).toLowerCase(), {
      personId: text(state?.personId || id).toLowerCase(),
      displayName: text(state?.displayName || state?.name || ''),
      lastKnownBeliefs: Array.isArray(state?.lastKnownBeliefs) ? state.lastKnownBeliefs.map(text).filter(Boolean) : [],
...
function normalizeModalityMetadata(input = {}, activeSpeakerId = '', activeCharacterId = '') {
  const modality = plainObject(input);
  const sourceModality = text(modality.sourceModality || modality.inputType || modality.input || 'text').toLowerCase() || 'text';
  const rawChannel = text(modality.sourceChannel || modality.channel || 'chat').toLowerCase() || 'chat';
  const sourceChannel = rawChannel === 'text' ? 'chat' : rawChannel;
  return {
    sourceModality,
    sourceChannel,
    speakerId: text(modality.speakerId || activeSpeakerId || ''),
    activeSpeakerId: text(modality.activeSpeakerId || activeSpeakerId || ''),
    recognizedPersonId: text(modality.recognizedPersonId || activeCharacterId || ''),
    speakerConfidence: Number(modality.speakerConfidence || 0) || undefined
  };
}
...
function createAishaStudioPulseRequest(input = {}) {
  ...
  const activeCharacterId = text(input.activeCharacterId || '');
  const activeSpeakerId = text(input.activeSpeakerId || input.activeCharacterId || '');
  const messageText = text(input.messageText || input.message || input.question || '');
  const modalityMetadata = normalizeModalityMetadata(input.modalityMetadata || input.modality, activeSpeakerId, activeCharacterId);
  return {
    sessionId,
    ...
    modality: {
      ...
      input: 'text',
      output: 'text',
      sourceModality: modalityMetadata.sourceModality,
      sourceChannel: modalityMetadata.sourceChannel,
