...
function summarizeRuntimeOptions(options = {}) {
  ...
  const timeoutMs = Number.isFinite(Number(options.productionGeminiTimeoutMs))
    ...
    : 0;
  const model = String(options.productionGeminiModel || '').trim();
  return {
    ...
    modelUsed: model || 'runtime-default',
  };
}

function normalizePersistenceDiagnostics(source = {}) {
  const mode = String(source.aishaPersistenceMode || '').trim().toLowerCase() === 'postgres'
    ? 'postgres'
    : 'memory';
  const backendRaw = String(source.aishaPersistenceBackend || '').trim().toLowerCase();
  const backend = ['in-memory', 'postgres', 'unavailable'].includes(backendRaw)
    ? backendRaw
    : (source.aishaPersistenceConnected === true ? (mode === 'postgres' ? 'postgres' : 'in-memory') : 'unavailable');
  return {
    aishaPersistenceMode: mode,
    aishaPersistenceBackend: backend,
    aishaPersistenceConnected: source.aishaPersistenceConnected === true,
    aishaPersistenceFailureReason: safeDiagnosticText(source.aishaPersistenceFailureReason || '')
  };
}
...
function baseDiagnostics(request = {}) {
  return {
    ...
    ...normalizePersistenceDiagnostics(),
    modelUsed: '',
    rejectionReason: ''
  };
}
...
async function callAishaEngine(input = {}, runtimeOptions = {}) {
  ...
  try {
    ...
    if (String(runtimeOptions.productionGeminiModel || '').trim()) {
      hostOptions.productionGeminiModel = String(runtimeOptions.productionGeminiModel || '').trim();
    }
    ...
    Object.assign(
      diagnostics,
      normalizePersistenceDiagnostics(response.trace?.aishaDiagnostics || response.diagnostics || {})
    );
