...
function buildIdentityQaPrompt(body = {}) {
  return [
    ...
    'Return one JSON object only: {"identityScore": number from 0 to 100, "identityVerdict": "looks_aligned" | "uncertain" | "mismatch_suspected", "mismatchNotes": string[]}.',
    'This is advisory QA only. Do not call the output accepted; the human reviewer decides whether it is gallery-ready.',
    'Use these thresholds: looks_aligned is 82-100, uncertain is 65-81, mismatch_suspected is below 65.',
...
function normalizeIdentityVerdict(verdict, score) {
  if (Number.isFinite(score)) {
    if (score >= 82) return 'looks_aligned';
  ...
  if (jsonMatch) {
    try {
      ...
      return {
        ...
        identityVerdict: normalizeIdentityVerdict(parsed.identityVerdict, normalizedScore),
        mismatchNotes: Array.isArray(parsed.mismatchNotes) ? parsed.mismatchNotes.map(item => String(item)).slice(0, 6) : []
      };
    } catch (_) {}
  }
  return {
    ...
    mismatchNotes: raw ? [raw.slice(0, 500)] : ['Identity QA returned no structured result.']
  };
}
