...
test('Prompt Generator V3 visual layer is visible, collapsed where needed, and red is restrained', () => {
  ...
  assert.match(script, /Prompt Generator atelier console/);
  assert.match(script, /pg50-model-drawer/);
  assert.match(script, /pg52-production-status-strip/);
  assert.match(script, /pg52-v3-shell/);
  assert.match(script, /pg52-v3-command-bar/);
  assert.match(script, /pg52-v3-char-block/);
  assert.match(script, /pg52-v3-ref-block/);
  assert.match(script, /pg52-v3-status-strip/);
  assert.match(script, /pg52-v3-layout/);
  assert.match(script, /pg52-v3-identity-data/);
  assert.match(script, /pg52-v3-canvas/);
  assert.match(script, /pg52-v3-canvas-hero/);
  assert.match(script, /pg52-step-shot/);
  assert.match(script, /pg52-step-scene/);
  assert.match(script, /pg52-step-styling/);
  assert.match(script, /pg52-v3-output-zone/);
  assert.match(script, /pg52-launch-prompt-preview/);
  assert.match(script, /pg52-launch-quality/);
  assert.match(script, /pg52-launch-readiness/);
  assert.match(script, /pg52-launch-generate/);
  assert.match(script, /pg52-v3-footer-bar/);
  assert.match(script, /pg52-v3-route-section/);
  assert.match(script, /pg52-v3-generate-section/);
  assert.match(script, /pg52-v2-history-strip/);
  assert.match(script, /pg52-v2-generate-readiness/);
  const modernShellStart = script.indexOf('function renderGeneratorShell52');
  const modernShellEnd = script.indexOf('function renderGeneratorShell51', modernShellStart);
  const modernShellBody = script.slice(modernShellStart, modernShellEnd);
  assert.doesNotMatch(modernShellBody, /workflowStripHtml\(/);
  assert.doesNotMatch(modernShellBody, /Left lane|Center lane|Right lane|LEFT LANE|CENTER LANE|RIGHT LANE/);
  assert.doesNotMatch(modernShellBody, /pg52-route-skeleton/);
  assert.doesNotMatch(script, /Left lane · Character context|Center lane · Shot Studio|Right lane · Output/);
  assert.match(script, /pg52-shot-mode-grid/);
  assert.match(script, /pg52-sa-moment-grid/);
  ...
  assert.match(script, /pg52-shot-canvas/);
  assert.match(script, /pg52-output-rail/);
  assert.match(script, /id="g-attach-refs"/);
  assert.match(script, /id="g-route-preview"/);
  assert.match(script, /id="g-image-model"/);
  assert.match(script, /id="g-spend-lane"/);
  assert.match(script, /onclick="generateImageFromGenerator\(this\)"/);
  assert.match(script, /onclick="generatePromptFromGenerator\(this\)"/);
  ...
  assert.match(css, /Prompt Generator V2\.2/);
  assert.match(css, /pg52-production-status-strip/);
  assert.match(css, /pg52-v3-shell/);
  assert.match(css, /pg52-v3-command-bar/);
  assert.match(css, /pg52-v3-char-block/);
  assert.match(css, /pg52-v3-ref-block/);
  assert.match(css, /pg52-v3-status-strip/);
  assert.match(css, /pg52-v3-canvas/);
  assert.match(css, /pg52-v3-canvas-hero/);
  assert.match(css, /pg52-creative-step/);
  assert.match(css, /pg52-shot-mode-grid/);
  assert.match(css, /pg52-sa-moment-grid/);
  assert.match(css, /pg52-v3-output-zone/);
  assert.match(css, /pg52-v3-footer-bar/);
  assert.match(css, /pg52-v3-generate-section/);
  assert.match(css, /pg52-launch-card/);
  assert.match(css, /pg52-launch-ready-row/);
  assert.match(css, /pg52-v2-history-strip/);
  assert.match(css, /pg52-v2-generate-readiness/);
...
test('public Studio Pulse showcase ships as a slim iframe-safe page', () => {
  ...
  assert.match(script, /function parentTargetOrigin/);
  assert.doesNotMatch(script, /return ['"]\*['"]/);
  assert.doesNotMatch(script, /postMessage\([^;]+,\s*['"]\*['"]/);
  ...
  assert.match(script, /Reference this/);
  assert.match(script, /reference-strip/);
  assert.match(script, /references: normalizeReferenceState/);
  assert.match(script, /reference-button/);
  ...
  assert.match(script, /SHOWCASE_VERSION = '1\.10\.0'/);
  ...
  assert.match(script, /turnRequestBody\(text, priorSpeaker, priorRecentTurns, turnReferences\)/);
  ...
  assert.match(turnGauntlet, /quick salad or a sandwich/);
  assert.match(turnGauntlet, /draft a three-stage agenda/);
  assert.match(turnGauntlet, /functional requirement remains/);
  ...
  assert.match(script, /pack1-memory/);
  assert.match(script, /showcase-session/);
  ...
  assert.match(css, /\.ledger-status-active/);
  assert.match(css, /\.ledger-status-superseded/);
  assert.match(css, /\.ledger-status-disputed/);
  ...
  assert.match(css, /\.message-reference/);
  assert.match(css, /\.reference-strip/);
  assert.match(css, /\.reference-chip/);
  ...
  assert.match(smoke, /backend CORS preflight/);
  assert.match(localSmoke, /__pulse-wrapper/);
  assert.match(localSmoke, /PULSE_SET_MODE/);
  assert.match(localSmoke, /PULSE_RESET/);
  assert.match(localSmoke, /PULSE_TURN_STATE/);
