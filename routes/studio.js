...
const SOCIAL_DIRECTOR_DEADLINE_MS = 26000;
...
function resolveSocialDirectorRuntimeOptions(providerConfig = {}) {
  ...
  options.socialDirectorDeadlineMs = SOCIAL_DIRECTOR_DEADLINE_MS;
  options.socialDirectorAttemptTimeoutMs = AISHA_PRODUCTION_GEMINI_TIMEOUT_MS;
  return options;
}
...
const PULSE_SHOWCASE_VISIBLE_HISTORY_LIMIT = 18;
const PULSE_SHOWCASE_VISIBLE_HISTORY_TTL_MS = 45 * 60 * 1000;
...
function __getPulseShowcaseVisibleHistoryForTests(sessionId = '') {
  return pulseShowcaseVisibleRecentTurns(sessionId, []);
}
...
function pulseShowcaseVisibleRecentTurns(sessionId = '', clientRecentTurns = []) {
  const id = pulseShowcaseSessionId(sessionId);
  prunePulseShowcaseVisibleHistories();
  const stored = pulseShowcaseVisibleHistories.get(id)?.turns || [];
  return dedupeShowcaseRecentTurns([...stored, ...sanitizeShowcaseRecentTurns(clientRecentTurns)]);
}

function recordPulseShowcaseVisibleHistory(sessionId = '', userText = '', messageEvents = []) {
  const id = pulseShowcaseSessionId(sessionId);
  const now = Date.now();
  prunePulseShowcaseVisibleHistories(now);
  const existing = pulseShowcaseVisibleHistories.get(id)?.turns || [];
  const additions = [
    { speakerId: 'user', role: 'user', text: userText },
  ...
  const turns = dedupeShowcaseRecentTurns([...existing, ...additions]);
  pulseShowcaseVisibleHistories.set(id, {
    turns,
...
function showcaseContinuityQualityContext(continuityProof = {}, continuityLedger = []) {
  ...
  return {
    ...
    activeTexts: rows
      .filter(item => item?.status === 'active')
      .map(item => safeShowcaseText(item.text || '', 240))
      .filter(Boolean)
      .slice(0, 4),
    supersededTexts: rows
      .filter(item => item?.status === 'superseded' || item?.status === 'disputed')
      .map(item => safeShowcaseText(item.text || '', 240))
      .filter(Boolean)
      .slice(0, 4)
  };
...
function showcaseExpansionTemplates(speakerId = '', anchor = '', roomMood = '') {
  ...
  const mood = roomMood ? `${roomMood[0].toUpperCase()}${roomMood.slice(1)} pace` : 'Steady pace';
  return {
    aisha: [
      `Receipt: ${subject}.`,
      'Cut the decorative version and keep the claim someone can verify.',
      'Name what would prove it true, then keep everything else out of the way.',
      `${mood}: keep the decision legible.`
    ],
    vanya: [
      `Temperature read: ${subject}.`,
      'Keep the first step small enough to do today.',
      'If it lands well, the user should feel less trapped and more directed.',
      `${mood}: lower the noise and keep one useful action in view.`
    ],
    leah: [
      ...
      `${mood}: keep the bite aimed at the weak point.`
    ],
    claudia: [
      ...
      `${mood}: reduce drift without inventing a process.`
    ],
    grok: [
      `Boundary fault: ${subject}.`,
      'Either verify the claim or admit the gap.',
      'Smallest real fix: remove the lie, then test the visible behavior.',
      `${mood}: keep the mechanism honest.`
    ]
  }[speakerId] || [
...
function sanitizeShowcaseRoomState(value = {}, mode = 'social_hierarchy_lab') {
  ...
  return {
    ...
    socialSignals: sanitizeShowcaseIncomingSocialSignals(source.socialSignals || {}),
    showcaseMode: mode
  };
}
...
function writePulseShowcaseSse(res, event, data = {}) {
...
router.post('/pulse-showcase/turn-stream', async (req, res) => {
  ...
  try {
    writePulseShowcaseSse(res, 'turn_start', {
      sessionId: parsed.sessionId,
      mode: parsed.mode,
    ...
    writePulseShowcaseSse(res, 'social_signals', projectShowcaseSocialSignals({
      mode: parsed.mode,
      roomMood: parsed.roomState.roomMood || (parsed.mode === 'continuity_breaker' ? 'sharp' : 'focused'),
      responseMode: parsed.roomState.responseMode || 'single',
      messageEvents: [],
      silentReactions: [],
      continuityLedger: [],
