...
const REJECTED_RX = /\b(objective is clear|not discussing|personal fitness routines|not the objective|focus is required|all systems nominal|current episode parameters|human temperature is stable|aesthetic standards are holding|no blandness detected|operational flow is clear|next steps are defined|no immediate faults detected|emergent anomalies|that's a solid goal|that's a great goal|that's a great way to begin|three days a week is a solid start|three training days a week is a solid start|fuel yourself well|eating enough to support|enough sleep to support|support that growth|get your sleep|one compound lift and one accessory movement|muscles huh|let'?s get you started|bodyweight basics|bodyweight exercises|eating enough protein|consistent effort|miracles overnight|track your lifts|measuring progress|just guessing|hydrate|workout buddy|don'?t overcomplicate it initially|just show up|show up and do the work|compound movements|multiple muscle groups|focused session|alternate between upper body and lower body|technically sound|poor form|fast track to injury|time constraint sharpens|current priorities|operational parameters|feedback is noted|perform usefulness|style guide|technical specs)\b/i;
