...
__export(index_exports, {
  __createPostgresProductionStoresForTests: () => __createPostgresProductionStoresForTests,
  __resetProductionDepsForTests: () => __resetProductionDepsForTests,
  processAishaRequest: () => processAishaRequest
});
...
var SOCIAL_DIRECTOR_LINE_JOB_RULES = [
...
var SOCIAL_DIRECTOR_ATTRIBUTION_TARGET_LINES = [
  {
...
function targetLinesForScenario(scenario) {
  return SOCIAL_DIRECTOR_ATTRIBUTION_TARGET_LINES.filter((item) => item.scenario === scenario).map((item) => `${speakerDisplayName(item.speakerId)} says '${item.text}'`).join(" ");
}
...
function formatSelectedSpeakerLineJobs(value) {
  ...
  const selected = Array.isArray(impulsePlan["selectedSpeakers"]) ? impulsePlan["selectedSpeakers"].map((item) => {
  ...
  const order = Array.isArray(impulsePlan["speakerOrder"]) ? impulsePlan["speakerOrder"].map((item) => asString(item)).filter(Boolean) : [];
...
function formatCurrentTurnAcceptanceTarget(value) {
  const socialDirector = asRecord(value);
  if (!socialDirector) return "";
  const impulsePlan = asRecord(socialDirector["impulsePlan"]);
  ...
  if (/\b(what is the objective|bruh|bro|wtf|where do i start)\b/.test(currentTurn) && /\b(muscle|muscles|fitness|workout|training|push-ups|pushups|rows|squats?|hinges?|plank|reps?|training days?)\b/.test(currentTurn)) {
...
var AishaPostgresConnection = class {
  pool;
  transactionClient = new import_node_async_hooks.AsyncLocalStorage();
  constructor(config) {
...
function productionKeyFingerprint(apiKey) {
  ...
  const digest = (0, import_node_crypto.createHash)("sha256").update(key).digest("hex").slice(0, 10);
