...
function formatCurrentTurnAcceptanceTarget(value: unknown): string {
  ...
  const selectedLineJobs = Array.isArray(impulsePlan?.["selectedSpeakers"])
    ? impulsePlan["selectedSpeakers"].map((item) => readString(asRecord(item), "lineJob") ?? "").join("\n").toLowerCase()
    : "";
  const currentTurn = [
  ...
  if (/\b(only have|only got|just have)\s+(20|twenty)\s+minutes?\b/.test(currentTurn)
    && /\b(record total reps|wall push-ups|glute bridges|5 minutes for plank|3 minutes warm-up|squats, wall push-ups)\b/.test(currentTurn)) {
  ...
  if (/\b(what is the objective|bruh|bro|wtf|where do i start)\b/.test(currentTurn)
    && /\b(muscle|muscles|fitness|workout|training|push-ups|pushups|rows|squats?|hinges?|plank|reps?|training days?)\b/.test(currentTurn)) {
  ...
  if (/\b(20|twenty)[-\s]+minute\b/.test(currentTurn)
    && /\b(referenced workout|fitness follow-up|workout follow-up|compress the referenced workout|movement categories|push|pull|hinge|core|reps?)\b/.test(currentTurn)) {
  ...
  if (/\b(no|never said|did i say|did i ever say)\b/.test(currentTurn)
    && /\b(black glass|white editorial|red pulse|no red|obsidian|pale blue|red accent|dashboard|landing page|preference|style)\b/.test(currentTurn)) {
